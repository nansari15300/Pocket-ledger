    ...
    async (alertCompanyId: string, voucherId: string, notificationTimestamp?: any, changedByUid?: string) => {
      ...
      try {
        ...
        if (snap.exists()) {
          ...
          setHistoryHighlightTimestamp(notificationTimestamp ?? null);
          setHistoryHighlightUid(changedByUid);
  ...
  return (
    <div className="px-[2px] py-4 sm:py-6 md:py-8 flex flex-col h-full w-full max-w-full">
        ...
        <HistoryDialog
          ...
          onOpenChange={(open) => { if (!open) { setHistoryVoucher(null); setHistoryHighlightTimestamp(null); setHistoryHighlightUid(undefined); } }}
