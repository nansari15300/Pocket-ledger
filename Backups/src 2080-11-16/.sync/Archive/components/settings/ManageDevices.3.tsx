  ...
  useEffect(() => {
    ...
    Promise.all(
      userIds.map(async (uid) => {
        ...
        if (snap.exists()) {
          const d = snap.data();
          const raw = d?.displayName?.trim() || d?.name?.trim() || d?.email || "";
              ...
              <TableBody>
                {devices.map((device) => (
                  <TableRow key={device.id}>
                    ...
                    <TableCell className="text-sm truncate max-w-[200px]" title={(() => {
                      const v = userNames[device.userId] ?? "—";
                      return (v === device.userId || /^[a-zA-Z0-9]{20,32}$/.test(v)) ? undefined : v;
                    })()}>
                      {(() => {
                        const v = userNames[device.userId] ?? "—";
                        if (v === device.userId || /^[a-zA-Z0-9]{20,32}$/.test(v)) return "—";
                        return v;
                      })()}
                    </TableCell>
                    ...
                    <TableCell className="text-muted-foreground">
                      {device.deviceType === "mobile" ? (
                        ...
                        <Monitor className="h-4 w-4 inline" title={device.deviceType === "desktop" ? "PC" : "Device"} />
                      )}
                    </TableCell>
