  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(devicesRef, (snap) => {
      const rows: DeviceRow[] = snap.docs.map((d) => {
        ...
        const lastActive = la ? (typeof la.toDate === "function" ? la.toDate() : new Date(la.toMillis())) : null;
        return { id: d.id, userId: data?.userId ?? "", lastActive };
      });
              ...
              <TableBody>
                {devices.map((device) => (
                  <TableRow key={device.id}>
                    ...
                    <TableCell className="text-muted-foreground text-sm truncate max-w-[120px]" title={device.userId}>
