...
export function HistoryDialog({ voucher, isOpen, onOpenChange, onHistoryReset, highlightTimestamp, highlightUid }: { voucher: any; isOpen: boolean; onOpenChange: (v: boolean) => void; onHistoryReset?: () => void; highlightTimestamp?: any; highlightUid?: string; }) {
  ...
  // Reset local history whenever the voucher changes (e.g. dialog reopened with fresh data)
  useEffect(() => { setLocalHistory(null); }, [voucher?.id]);

  const history = ((localHistory ?? voucher?.history) || [])
  ...
  const handleDeleteEntry = async (entry: any, idx: number) => {
    ...
    try {
      ...
      // Update local history — only remove the deleted entries, keep the rest
      const toDelete = new Set(msValues);
      const currentRaw = localHistory ?? voucher?.history ?? [];
      setLocalHistory(currentRaw.filter((h: any) => {
        const c = h.changedAt;
        const ms = c?.toDate instanceof Function ? c.toDate().getTime()
          : c?._seconds != null ? c._seconds * 1000
