...
function EntriesChangesRows({ from, to, renderValue, mode = 'amounts' }: { from: any[]; to: any[]; renderValue: (v: any) => React.ReactNode; mode?: EntriesMode }) {
  ...
  const entryTypeSortBase = (type: string, key: string): number => {
    if (key === 'accountId') {
      if (type === 'expense') return 4;   // Debit Account — amounts slot, just before Salary Amount
      if (type === 'staff')   return 2;   // Staff Member — ids slot
      if (type === 'tax')     return 3;   // Tax Account — ids slot
      return 4;
    }
