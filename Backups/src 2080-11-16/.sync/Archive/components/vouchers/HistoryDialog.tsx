...
function getEntryType(entry: any): 'approve' | 'unapprove' | 'edit' {
  const allKeys = Object.keys(entry.changes || {});
  const meaningfulKeys = allKeys.filter(k =>
    k !== 'approvedByUserId' && k !== 'approvedBy' && k !== 'lastEditedByUserName'
  );
  const allApprovalRelated = meaningfulKeys.length > 0 && meaningfulKeys.every(k => APPROVAL_FIELDS.has(k));
  if (allApprovalRelated) {
    const isApprovedChange = entry.changes['IsApproved'] ?? entry.changes['isApproved'];
    if (isApprovedChange) {
      return isApprovedChange.to === true ? 'approve' : 'unapprove';
    }
    return 'approve';
  }
  return 'edit';
          ...
          <div className="space-y-6 p-1 min-w-[480px]">
            {history.length > 0 ? (
              history.map((entry: any, i: number) => {
                ...
                const badgeClass =
                  entryType === 'approve' ? "bg-green-600 text-white" :
                  entryType === 'unapprove' ? "bg-rose-600 text-white" :
                  "bg-blue-600 text-white";
                const badgeLabel =
                  entryType === 'approve' ? "Approved" :
                  entryType === 'unapprove' ? "Unapproved" :
                  "Edited";
                return (
                  ...
                  <Table className="w-full min-w-[480px]" scrollContainer={false}>
                    ...
                    <TableBody>
                      {Object.entries(entry.changes).map(([field, values]: [string, any]) => {
                        ...
                        if (field === 'approvedByUserId' || field === 'approvedBy' || field === 'lastEditedByUserName') return null;
                        ...
                        // Skip rows where old and new values are identical (no real change)
                        const rawFrom = values.from;
                        const rawTo = displayTo;
                        const isTimestampLike = (v: any) => v?.toDate instanceof Function || (v?._seconds !== undefined);
                        if (!isTimestampLike(rawFrom) && !isTimestampLike(rawTo) && JSON.stringify(rawFrom) === JSON.stringify(rawTo)) return null;

                        const fieldLabel = field === 'lastEditedBy' ? 'Last edited by' : field === 'lastEditedAt' ? 'Last edited date and time' : field === 'date' ? 'voucher Date' : field;
                        return (
                          <TableRow key={field}>
