...
export function useDeviceLimit() {
  ...
  // When this device is removed from the list (e.g. replaced by another device), run check immediately so overlay shows live
  useEffect(() => {
    ...
    const unsub = onSnapshot(deviceRef, (snap) => {
      if (!snap.exists()) runCheckRef.current?.();
    });
    return () => unsub();
  }, [companyId, user?.uid]);
