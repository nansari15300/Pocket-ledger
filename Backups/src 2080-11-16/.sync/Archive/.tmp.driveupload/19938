...
/** Display order for history fields (lower = shown first). Unknown fields get order 999. */
const FIELD_ORDER: Record<string, number> = {
  voucherNumber: 1,
  date: 2,
  partyId: 3,
  accountId: 4, debitAccountId: 5, fromAccountId: 6, toAccountId: 7,
  lineItems: 8,
  entries: 9, Entries: 9,
  subTotal: 10,
  discount: 11,
  ...
  narration: 15,
  linkedVoucherNo: 16,
  UserDisplayName: 17, userDisplayName: 17,
  fileUrls: 18, fileUrl: 18, url: 18,
  UserEmail: 19,
  ApprovedByUserName: 20, approvedByUserName: 20,
  approvedAt: 21, ApprovedAt: 21,
  IsApproved: 22, isApproved: 22,
  lastEditedBy: 23,
  lastEditedAt: 24,
};
                  ...
                  <Table className="w-full min-w-[480px]" scrollContainer={false}>
                    ...
                    <TableBody>
                      {(Object.entries(entry.changes) as [string, any][])
                        ...
                        // --- skip rows with no real change ---
                        const isTimestampLike = (v: any) => v?.toDate instanceof Function || (v?._seconds !== undefined);
                        if (!isTimestampLike(values.from) && !isTimestampLike(displayTo) && JSON.stringify(values.from) === JSON.stringify(displayTo)) return null;
