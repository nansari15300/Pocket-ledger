...
function EntriesChangesRows({ from, to, renderValue }: { from: any[]; to: any[]; renderValue: (v: any) => React.ReactNode }) {
  ...
  // 2. Removed entries — ONE "Staff Member" row (old→paired-new or old→N/A) + sub-fields
  for (const rawOld of removedArr) {
    ...
    pushSubFields(rawOld, null, `${id}-rem`);
  }

  // 3. Added entries — if paired, skip "Staff Member" row (already shown above); show sub-fields only
  //    If not paired, show "Staff Member" row too
  for (const rawNew of addedArr) {
    const id = rawNew.accountId;
    if (!pairedAddedIds.has(id)) {
      const { type } = resolveEntryInfo(id);
      const labels = ENTRY_FIELD_LABELS[type] ?? ENTRY_FIELD_LABELS.other;
      rows.push({
        key: `${id}-accountId`,
