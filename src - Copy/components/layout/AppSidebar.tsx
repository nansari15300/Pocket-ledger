...
export function AppSidebar() {
  ...
  const showApproveInSidebar =
    ...
    company?.notificationSettings?.approve?.onEntity === true;
  ...
  const pendingCountByEntity = useMemo(() => {
    ...
    out.party = pending.filter((v: any) => v.partyId).length;
    out['bank-cash'] = pending.filter((v: any) => v.fromAccountId || v.toAccountId || v.accountId).length;
    out.staff = pending.filter((v: any) => v.staffId).length;
    out.tax = pending.filter((v: any) =>
      v.taxAccountId || (Array.isArray(v.lineItems) && v.lineItems.some((l: any) => l.taxAccountId))
    ).length;
    out.items = pending.filter((v: any) =>
      Array.isArray(v.lineItems) && v.lineItems.some((l: any) => l.itemId)
    ).length;
    out.incomes = pending.filter((v: any) =>
      v.incomeAccountId || v.expenseAccountId || (Array.isArray(v.entries) && v.entries.some((e: any) => e.accountId))
    ).length;
    return out;
  }, [vouchers, showApproveInSidebar]);
  ...
  useEffect(() => {
    ...
    // This will store unsubscribe functions for each conversation's message listener
    ...
    const unsubConversations = onSnapshot(conversationsQuery, (convSnap) => {
      // Clean up old message listeners before creating new ones
  ...
  const includeAlertsInSidebar = transactionAlerts?.on === true && transactionAlerts?.onEntity === true;
  ...
  const showMessageBadgeInSidebar =
    (company?.notificationSettings?.message?.on === true &&
      company?.notificationSettings?.message?.onEntity === true) ||
    includeAlertsInSidebar;
      ...
      <SidebarContent>
        ...
        <SidebarMenu>
          {visibleMenuItems.map((item) => {
            ...
            const showPendingBadge = isOpen && pendingCount > 0;
            ...
            <SidebarMenuItem key={item.href}>
              <Link href={item.href} passHref>
                  ...
                  >
                      ...
                      {showPendingBadge && (
                        <span className="ml-auto h-5 min-w-[1.25rem] px-1 flex items-center justify-center rounded-full bg-pink-500 text-white text-xs font-medium">
                          {pendingCount}
                        </span>
                      )}
                  </SidebarMenuButton>
                        ...
                        <SidebarMenuItem key={item.href}>
                            <Link href={item.href} passHref>
                                ...
                                >
                                    ...
                                    {isOpen && <span className="flex-1">{item.label}</span>}
                                     {isOpen && showBadge && (
                                        <span className="ml-auto h-5 w-5 flex items-center justify-center rounded-full bg-red-500 text-white text-xs">{badgeCount}</span>
                                    )}
                                </SidebarMenuButton>
                            </Link>
                        </SidebarMenuItem>
