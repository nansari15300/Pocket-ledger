...
import { Download, Upload, Loader2, FileWarning, KeyRound, ShieldCheck, ShieldOff } from "lucide-react";
...
import { createCompanyFromBackup, sendSecurityAlert } from "@/lib/actions";
...
export function BackupRestore() {
  ...
  const handleBackup = async () => {
    ...
    try {
      ...
      const fileExtension = 'webtally';
      a.download = `webtally_backup_${company.name.replace(/\s+/g, '_')}_${timestamp}.${fileExtension}`;
  ...
  const processRestoreData = async () => {
    ...
    const isPotentiallyEncrypted = fileToRestore.name.endsWith('.webtally') || !fileContent.trim().startsWith('{');
  ...
  const handleOverwriteRestore = async (backupData: any) => {
    ...
    if (user.email !== backupOwnerEmail) {
        toast({
            variant: "destructive",
            title: "Restore Blocked",
            description: `This backup belongs to ${backupOwnerEmail}. Illegal restore attempt recorded.`,
            duration: 8000,
        });
        ...
        await sendSecurityAlert(
            ...
            user.uid,
            user.email ?? "",
            backupCompanyDetails.name,
            backupCompanyId,
            (customUser?.displayName || user?.displayName) ?? undefined
        );
      ...
      <div className="space-y-8">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Restore company data from a JSON or encrypted .webtally file. This will overwrite the current company&apos;s data.
            </CardDescription>
          </CardHeader>
          <CardContent className="flex items-center gap-4">
            <Input type="file" accept=".json,.webtally" onChange={handleFileSelect} />
          </CardContent>
          <CardFooter>
            <PermissionButton
