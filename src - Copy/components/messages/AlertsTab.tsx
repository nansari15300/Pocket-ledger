...
export function AlertsTab({
  ...
  /** When provided, "Open in edit" opens voucher in a popup on the same page instead of navigating. */
                ...
                notifications.map((n) => {
                    ...
                    const rows: { label: string; right: React.ReactNode }[] = [
                      ...
                      ...(!isSecurityAlert && !isAlarm && (voucherNo || amountStr) ? [{
                        label: voucherNo ?? "—",
                        right: amountStr ? <span className="text-sm font-medium">{amountStr}</span> : null,
                      }] : []),
