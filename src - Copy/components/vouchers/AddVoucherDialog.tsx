...
function VoucherDialogContent({ 
  ...
  useEffect(() => {
    setActiveTab(getVoucherType(voucher, defaultVoucherData, defaultTab));
  }, [voucher, defaultVoucherData, defaultTab]);
  ...
  const tabKeys = TAB_ORDER.filter((k) => k in formMap);
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      ...
      <div className={cn("px-[2px] pt-6 flex-1 flex flex-col min-h-0", isMobile ? "pb-0" : "pb-6 md:p-6")}>
...
export function AddVoucherDialog(props: any) {
  ...
  const handleApprove = useCallback(async () => {
    if (!companyId || !voucher?.id || isApproving) return;
    setIsApproving(true);
    try {
      await updateDoc(doc(firestore, `companies/${companyId}/vouchers`, voucher.id), { isApproved: true });
  ...
  }, [companyId, voucher?.id, isApproving, props.onVoucherAction, onOpenChange]);
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="flex flex-col p-0 w-[calc(100vw-4px)] md:w-[calc(100vw-40px)] max-w-7xl h-[calc(100vh-40px)] max-h-[90vh] rounded-lg md:rounded-lg">
        ...
        <VoucherDialogContent 
          ...
          onOpenHistory={effectiveVoucher?.id ? () => setHistoryVoucher(effectiveVoucher) : undefined}
