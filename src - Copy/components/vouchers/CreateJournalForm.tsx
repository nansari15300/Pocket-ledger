...
export function CreateJournalForm({
  ...
  const { dateSystem, formatCurrencyForPrint } = useDate();
...
async function handleFormSubmit(e: React.FormEvent, options: { saveAndNew?: boolean } = {}) {
  ...
  async function processAndSave(data: JournalFormValues, saveAndNew: boolean = false) {
    if (!user || !companyId) {
    ...
    <>
      <Form {...form}>
        <form onSubmit={(e) => handleFormSubmit(e)} className="h-full flex flex-col">
                    ...
                    <FormField
                      ...
                      render={({ field }: any) => (
                        <FormItem className="flex flex-col flex-1 min-w-0">
                          ...
                          <FormControl>
                            <Input placeholder="e.g. JV-001" {...field} className="h-9 text-xs px-2" disabled={isAutoVoucherEnabled && (!isVoucherEditingAllowed || !can('edit_voucher_numbers'))} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                              ...
                              <div className="flex-1 min-w-0">
                                <Popover open={isCalendarOpen} onOpenChange={setIsCalendarOpen} modal={true}>
                                  <PopoverTrigger asChild>
                                    <FormControl>
                                      <Button disabled={!isFormEditing} variant={"outline"} className={cn("h-9 pl-2 pr-2 text-left font-normal text-xs w-full", !field.value && "text-muted-foreground")}>
          ...
          {isFormEditing && <div className={cn("border-t", isMobile ? "mt-[3px] pt-[3px] pb-[3px]" : "pt-4 flex flex-col md:flex-row justify-between items-stretch md:items-center gap-4")}>
            {isMobile ? (
              <div className="grid grid-cols-3 gap-2 w-full [&_button]:h-10">
                ...
                <Button type="submit" disabled={isLoading || editingDisabled} className="w-full">
                  {isLoading ? "..." : (voucher ? "Update" : "Save")} Journal
                </Button>
