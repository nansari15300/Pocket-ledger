...
export function CreatePurchaseForm({
  ...
  const isRateEditingAllowed = (company?.allowRateEditing?.purchase ?? true) && canEditRates;
  ...
  const handleFormSubmit = useCallback(async (e: React.FormEvent, options: { saveAndNew?: boolean, print?: boolean } = {}) => {
      ...
      if (!isValid) {
          ...
          if (errors.partyId) errorMessages.push(`Supplier: ${errors.partyId.message}`);
          ...
          <ScrollArea className="flex-1 pr-6 -mr-6">
            <div className={cn(
              ...
              {isMobile ? (
                    ...
                    <FormField
                      ...
                      render={({ field }: any) => (
                        <FormItem className="flex flex-col flex-1 min-w-0">
                          <FormLabel className="text-xs">Invoice No.</FormLabel>
                          <div className="flex gap-1 h-9">
                            {isPrefixSelectionEnabled && voucherPrefixes.length > 0 ? (
                              <Select
                                ...
                                value={
                                  voucherPrefixes.find((p) =>
                                    field.value?.startsWith(p)
                                  ) || voucherPrefixes[0]
                                }
                              >
                            ...
                            <FormControl>
                              <Input
                                ...
                                disabled={
                                  ...
                                  (!isVoucherEditingAllowed || !can('edit_voucher_numbers'))
                                }
                              />
                            </FormControl>
                          </div>
                ...
                <>
                  {/* PC View: All 4 Fields in Same Row with Equal Spacing */}
                  <div className="flex flex-wrap items-end justify-between gap-4">
                    ...
                    <FormField
                      ...
                      name="partyId"
                      render={({ field }: any) => (
                        <FormItem className="flex flex-col min-w-[200px] flex-[1_1_200px] 2xl:flex-none">
                           <div className="flex justify-between items-baseline">
                            <FormLabel>Supplier (Cr.)</FormLabel>
                            {partyBalance !== null && partyBalance !== undefined && (
                              <FormLabel className={cn("text-xs font-semibold", partyBalance >= 0 ? 'text-green-600' : 'text-red-600')}>
                                {partyBalance >= 0 
                                    ? `Receivable: ${formatCurrencyForPrint(partyBalance, { noSuffix: true, noAnimation: true })} Dr`
                                    : `Payable: ${formatCurrencyForPrint(Math.abs(partyBalance), { noSuffix: true, noAnimation: true })} Cr`
                                }
                              </FormLabel>
                            )}
                          </div>
                    ...
                    />
                    {/* 2nd: Purchase Account (Dr.) - Wraps to next line if needed */}
                    <FormField
                      control={form.control}
                      name="purchaseAccountId"
                      render={({ field }: any) => (
                        <FormItem className="flex flex-col min-w-[200px] flex-[1_1_200px] xl:flex-none">
                          <div className="flex justify-between items-baseline">
                            <FormLabel>Purchase Account (Dr.)</FormLabel>
                          </div>
                          <div className="flex gap-2">
                            <Combobox
                              options={expenseAccounts
                    ...
                    <FormField
                      ...
                      name="voucherNumber"
                      render={({ field }: any) => (
                        <FormItem className="flex flex-col min-w-[180px] flex-shrink-0">
                    ...
                    />
                    {/* 4th: Date - Fixed 40mm width */}
                    <FormField
                      control={form.control}
                      name="date"
                      render={({ field }: any) => (
                        <FormItem className="flex flex-col flex-shrink-0" style={{ width: '40mm', maxWidth: '40mm' }}>
                          <FormLabel>Date</FormLabel>
                          <div className={cn(
                            "flex gap-2 h-10",
                            dateSystem === 'Both' && "gap-2",
                            "flex-wrap"
                          )}>
                            {(dateSystem === 'BS' || dateSystem === 'Both') && (
                              <div style={{ width: '40mm', maxWidth: '40mm', flexShrink: 0 }}>
                                <BsDatePicker valueAD={field.value} onChangeAD={(d) => {
                                  if (d) d.setHours(12, 0, 0, 0);
                                  field.onChange(d as Date);
                                  setIsCalendarOpen(false);
                                }} isRange={false} transactionDates={transactionDates} className="h-10 w-full" />
                              </div>
                            )}
                            {(dateSystem === 'AD' || dateSystem === 'Both') && (
                              <Popover open={isCalendarOpen} onOpenChange={setIsCalendarOpen}>
                                <PopoverTrigger asChild>
                                  <FormControl>
                                    <Button variant={"outline"} className={cn("h-10 pl-3 text-left font-normal", !field.value && "text-muted-foreground", "flex-shrink-0")}>
