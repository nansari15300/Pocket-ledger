...
const getPayeeTypeFromVoucher = (v: any) => {
  ...
  if (v?.type === 'direct_expense') {
    if (v?.payToType === 'OTHER' || v?.toAccountId) return 'other';
...
export function CreatePaymentOutForm({
  ...
  useEffect(() => {
    ...
    } else if (voucherType === 'direct_expense' && !['expense', 'other'].includes(payeeType)) {
        form.setValue('payeeType', 'expense');
    }
  }, [payeeType, voucherType, form]);
  
  async function handleFormSubmit(e: React.FormEvent, options: { saveAndNew?: boolean, print?: boolean } = {}) {
    e.preventDefault();
    ...
    processAndSave(form.getValues(), options.saveAndNew, options.print);
  }
  
  async function processAndSave(data: PaymentOutFormValues, saveAndNew: boolean = false, print: boolean = false) {
    if (!user || !companyId) {
    ...
    try {
      ...
      if (voucherType === 'direct_expense') {
        ...
        const payTo = submissionData.payeeType === 'other' ? 'OTHER' : 'EXPENSE';
        submissionData.payToType = payTo;
        submissionData.toAccountId = payTo === 'EXPENSE' ? (submissionData.expenseAccountId || submissionData.toAccountId) : (submissionData.toAccountId || submissionData.expenseAccountId);
        if (payTo === 'EXPENSE') submissionData.expenseAccountId = submissionData.toAccountId;
      ...
      if (newFilesToUpload.length > 0) {
        ...
        for (const file of newFilesToUpload) {
          if (sanitizedData.fileUrls.length >= 3) break;
        ...
        <form onSubmit={(e) => handleFormSubmit(e)} className="h-full flex flex-col">
          <ScrollArea className="flex-1 pr-6 -mr-6">
            <div className={cn(
                      ...
                      render={({ field }: any) => (
                        <FormItem className="flex flex-col flex-1 min-w-0">
                          ...
                          <div className="flex gap-1 h-9">
                            ...
                            <FormControl>
                              <Input placeholder="e.g. PYMT-001" {...field} className="h-9 text-xs px-2" disabled={isAutoVoucherEnabled && (!isVoucherEditingAllowed || !can('edit_voucher_numbers'))} />
                            </FormControl>
                          </div>
                          <FormMessage />
                        </FormItem>
              ...
              {isMobile ? (
                <>
                  {/* Mobile: To and From accounts - 2 columns */}
                  <div className="flex gap-[2px] w-full">
                    {payeeType === 'party' && (
                      <FormField
                        control={form.control}
                        ...
                        render={({ field }: any) => (
                          <FormItem className="flex-1 min-w-0">
                            <div className="flex justify-between items-baseline mb-1">
                              <FormLabel className="text-xs">To (Party)</FormLabel>
                              {payeeBalance !== null && payeeBalance !== undefined && (
                                <FormLabel className={cn("text-[10px] font-semibold mr-[2px]", payeeBalance >= 0 ? 'text-green-600' : 'text-red-600')}>
                    ...
                    {payeeType === 'staff' && (
                      <FormField
                        ...
                        render={({ field }: any) => (
                          <FormItem className="flex-1 min-w-0">
                            <div className="flex justify-between items-baseline mb-1">
                              <FormLabel className="text-xs">To (Staff)</FormLabel>
                              {payeeBalance !== null && payeeBalance !== undefined && (
                                <FormLabel className={cn("text-[10px] font-semibold mr-[2px]", payeeBalance >= 0 ? 'text-green-600' : 'text-red-600')}>
                                  {formatCurrencyForPrint(payeeBalance, { noSuffix: true, noAnimation: true })} {payeeBalance >= 0 ? 'Dr' : 'Cr'}
                                </FormLabel>
                    ...
                    {payeeType === 'tax' && (
                      <FormField
                        ...
                        render={({ field }: any) => (
                          <FormItem className="flex-1 min-w-0">
                            <div className="flex justify-between items-baseline mb-1">
                              <FormLabel className="text-xs">To (Tax)</FormLabel>
                              {payeeBalance !== null && payeeBalance !== undefined && (
                                <FormLabel className={cn("text-[10px] font-semibold mr-[2px]", payeeBalance >= 0 ? 'text-green-600' : 'text-red-600')}>
                                  {formatCurrencyForPrint(payeeBalance, { noSuffix: true, noAnimation: true })} {payeeBalance >= 0 ? 'Dr' : 'Cr'}
                                </FormLabel>
                    ...
                    {payeeType === 'expense' && (
                      <FormField
                        ...
                        render={({ field }: any) => (
                          <FormItem className="flex-1 min-w-0">
                            <div className="flex justify-between items-baseline mb-1">
                              <FormLabel className="text-xs">To (Expense)</FormLabel>
                              {payeeBalance !== null && payeeBalance !== undefined && (
                                <FormLabel className={cn("text-[10px] font-semibold mr-[2px]", payeeBalance >= 0 ? 'text-green-600' : 'text-red-600')}>
                                  {formatCurrencyForPrint(payeeBalance, { noSuffix: true, noAnimation: true })} {payeeBalance >= 0 ? 'Dr' : 'Cr'}
                                </FormLabel>
                      ...
                      <>
                        <FormField
                          ...
                          render={({ field }: any) => (
                            <FormItem className="flex-1 min-w-0">
                              <div className="flex justify-between items-baseline mb-1">
                                <FormLabel className="text-xs">To Account (Other)</FormLabel>
                                {payeeBalance !== null && payeeBalance !== undefined && (
                                  <FormLabel className={cn("text-[10px] font-semibold mr-[2px]", payeeBalance >= 0 ? 'text-green-600' : 'text-red-600')}>
                                    {formatCurrencyForPrint(payeeBalance, { noSuffix: true, noAnimation: true })} {payeeBalance >= 0 ? 'Dr' : 'Cr'}
                                  </FormLabel>
                        ...
                        <FormField
                          ...
                          render={({ field }: any) => (
                            <FormItem className="flex-1 min-w-0">
                              <FormLabel className="text-xs">Payee Name (optional)</FormLabel>
                              <FormControl>
                                <Input placeholder="e.g. Advance, Deposit" {...field} className="h-9 text-xs" />
                              </FormControl>
                              <FormMessage />
                            </FormItem>
                          )}
                    ...
                    <FormField
                      ...
                      render={({ field }: any) => (
                        <FormItem className="flex-1 min-w-0">
                          <div className="flex justify-between items-baseline mb-1">
                            <FormLabel className="text-xs">From Bank/Cash</FormLabel>
                            {accountBalance !== null && <FormLabel className="text-[10px] text-muted-foreground">Bal: {formatCurrency(accountBalance, {noAnimation: true, noSuffix: true})}</FormLabel>}
                          </div>
                          <div className="flex gap-1">
                            <Combobox
                    ...
                    <>
                      <FormField
                        ...
                        render={({ field }: any) => (
                          <FormItem>
                            <div className="flex justify-between items-baseline">
                              <FormLabel>To Account (Other)</FormLabel>
                              {payeeBalance !== null && payeeBalance !== undefined && (
                                <FormLabel className={cn("text-xs font-semibold", payeeBalance >= 0 ? 'text-green-600' : 'text-red-600')}>
                                  {formatCurrencyForPrint(payeeBalance, { noSuffix: true, noAnimation: true })} {payeeBalance >= 0 ? 'Dr' : 'Cr'}
                                </FormLabel>
                            ...
                            <Combobox
                              ...
                              placeholder="Select account (advance, deposit, drawings, etc.)"
                              addNewLabel="+ Add New Account"
                            />
                            <FormMessage />
                          </FormItem>
                      ...
                      <FormField
                        ...
                        render={({ field }: any) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Input placeholder="e.g. Advance, Deposit" {...field} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
          ...
          <div className={cn(
            ...
            {isMobile ? (
              <div className="grid grid-cols-3 gap-2 w-full [&_button]:h-10">
                ...
                <Button type="button" onClick={(e) => handleFormSubmit(e, { print: true })} disabled={isLoading || editingDisabled} className="w-full">
                  Save & Print
                </Button>
                <Button type="submit" disabled={isLoading || editingDisabled} className="w-full">
                  {isLoading ? "..." : (voucher ? "Update" : "Save")} Payment
                </Button>
