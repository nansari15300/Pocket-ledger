  ...
  const fetchVoucherNumber = useCallback(async (selectedPrefix?: string) => {
    ...
    try {
        ...
        if (numStr && numStr.startsWith(prefix)) {
            const num = parseInt(numStr.replace(prefix, ""), 10);
            if (!isNaN(num) && num > maxNum) maxNum = num;
        }
        });

        const nextNumber = maxNum + 1;
        const nextVoucherNo = `${prefix}${nextNumber}`;
        form.setValue("voucherNumber", nextVoucherNo);
    } catch (error) {
...
async function processAndSave(data: SalaryFormValues, saveAndNew: boolean = false) {
        ...
        <form onSubmit={(e) => handleFormSubmit(e)} className="h-full flex flex-col">
          <ScrollArea className="flex-1 pr-6 -mr-6">
            <div className={cn(
              ...
              {isMobile ? (
                <>
                  {/* Mobile: Voucher No. and Date - Same Row, 40mm each */}
                                  ...
                                  <PopoverTrigger asChild>
                                    <FormControl>
                                      <Button 
                                        ...
                                        variant={"outline"} 
                                        className={cn("h-9 pl-2 pr-2 text-left font-normal text-xs w-full", !field.value && "text-muted-foreground")}
                                      >
                          ...
                          {fields.map((field, index) => {
                            ...
                            return (
                              <TableRow key={field.id}>
                                ...
                                <TableCell><FormField control={form.control} name={`lineItems.${index}.salary`} render={({ field }: any) => (<FormItem><FormControl><Input type="number" value={field.value || ''} onChange={(e) => { const value = e.target.value === '' ? 0 : parseFloat(e.target.value) || 0; field.onChange(value); }} onBlur={field.onBlur} /></FormControl><FormMessage /></FormItem>)}/></TableCell>
                                 <TableCell>
          ...
          {isFormEditing && (
            ...
            {isMobile ? (
              ...
              <>
                ...
                <div className="flex gap-2 md:gap-4 [&_button]:h-10">
                  {voucher && (
                    <Button type="button" variant="outline" onClick={() => onVoucherAction?.('cancelled')} className="w-full md:w-auto">
                      Cancel
                    </Button>
                  )}
                  {!voucher && (
                    <>
                      ...
                      <Button type="button" onClick={(e) => handleFormSubmit(e, { saveAndNew: true })} disabled={isLoading || editingDisabled} className="w-full md:w-auto">
                        {isLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                        Save & New
                      </Button>
                    </>
                  )}
                  <Button type="submit" disabled={isLoading || editingDisabled} className="w-full md:w-auto">
                    {isLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                    Save
                  </Button>
                  {showApproveButton && onApprove && (
                    <Button type="button" variant="default" onClick={onApprove} disabled={isApproving} className="w-full md:w-auto shrink-0">
