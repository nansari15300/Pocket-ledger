...
export function CreateProductionForm({
  ...
  const fetchVoucherNumber = useCallback(async () => {
    ...
    try {
      ...
      form.setValue("productionNumber", `${VOUCHER_PREFIX}${nextVoucherNumber}`);
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col h-full">
        ...
        <div className={cn("border-t", isMobile ? "mt-[3px] pt-[3px] pb-[3px] px-2" : "p-4 flex justify-between items-center")}>
          {isMobile ? (
            <div className="grid grid-cols-3 gap-2 w-full [&_button]:h-10">
              ...
              {isEditing && showApproveButton && onApprove && (
                <Button type="button" variant="default" onClick={onApprove} disabled={isApproving} className="w-full">
                  {isApproving ? "..." : "Approve"}
                </Button>
              )}
