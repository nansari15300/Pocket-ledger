...
export function CreatePaymentInForm({
  ...
  useEffect(() => {
    if (voucher || defaultVoucherData) {
        const source = voucher || defaultVoucherData;
        const initialValues = getInitialFormValues(source);
  ...
  useEffect(() => {
    ...
    } else if (voucherType === 'direct_income' && !['income', 'other'].includes(payeeType)) {
  ...
  async function processAndSave(data: PaymentInFormValues, saveAndNew: boolean = false, print: boolean = false) {
    if (!user || !companyId) {
        ...
        <form onSubmit={(e) => handleFormSubmit(e)} className="h-full flex flex-col">
          <ScrollArea className="flex-1 pr-6 -mr-6">
            <div className={cn(
                      ...
                      render={({ field }: any) => (
                        <FormItem className="flex flex-col flex-1 min-w-0">
                          ...
                          <div className="flex gap-1 h-9">
                            ...
                            <FormControl>
                              <Input placeholder="e.g. RCPT-001" {...field} className="h-9 text-xs px-2" disabled={isAutoVoucherEnabled && (!isVoucherEditingAllowed || !can('edit_voucher_numbers'))} />
                            </FormControl>
                          </div>
                          <FormMessage />
                        </FormItem>
              ...
              {isMobile ? (
                <>
                  {/* Mobile: From and To accounts - 2 columns */}
                  <div className="flex gap-[2px] w-full">
                    {payeeType === 'party' && (
                      <FormField
                        control={form.control}
                        ...
                        render={({ field }: any) => (
                          <FormItem className="flex-1 min-w-0">
                            <div className="flex justify-between items-baseline mb-1">
                              <FormLabel className="text-xs">From (Party)</FormLabel>
                    ...
                    {payeeType === 'staff' && (
                      <FormField
                        ...
                        render={({ field }: any) => (
                          <FormItem className="flex-1 min-w-0">
                            <div className="flex justify-between items-baseline mb-1">
                              <FormLabel className="text-xs">From (Staff)</FormLabel>
                              {payeeBalance !== null && payeeBalance !== undefined && (
                                <FormLabel className={cn("text-[10px] font-semibold mr-[2px]", payeeBalance >= 0 ? 'text-green-600' : 'text-red-600')}>
                                  {formatCurrencyForPrint(payeeBalance, { noSuffix: true, noAnimation: true })} {payeeBalance >= 0 ? 'Dr' : 'Cr'}
                                </FormLabel>
                    ...
                    {payeeType === 'tax' && (
                      <FormField
                        ...
                        render={({ field }: any) => (
                          <FormItem className="flex-1 min-w-0">
                            <div className="flex justify-between items-baseline mb-1">
                              <FormLabel className="text-xs">From (Tax)</FormLabel>
                            </div>
                            <div className="flex gap-1">
                              <Combobox
                    ...
                    {payeeType === 'income' && (
                      <FormField
                        ...
                        render={({ field }: any) => (
                          <FormItem className="flex-1 min-w-0">
                            <div className="flex justify-between items-baseline mb-1">
                              <FormLabel className="text-xs">From (Income)</FormLabel>
                            </div>
                            <div className="flex gap-1">
                              <Combobox
                      ...
                      <FormField 
                        ...
                        render={({ field }: any) => (
                          <FormItem className="flex-1 min-w-0">
                            ...
                            <FormControl>
                              <Input placeholder="Enter source name" {...field} className="h-9 text-xs" disabled={deleteDisabledWhenLinked} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )} 
                    ...
                    <FormField
                      ...
                      render={({ field }: any) => (
                        <FormItem className="flex-1 min-w-0">
                          <div className="flex justify-between items-baseline mb-1">
                            <FormLabel className="text-xs">To Bank/Cash</FormLabel>
                ...
                {payeeType === 'tax' && (
                 ...
                 {payeeType === 'other' && (
                    <FormField control={form.control} name="payeeName" render={({ field }: any) => (<FormItem><FormLabel>Source Name</FormLabel><FormControl><Input placeholder="Enter source name" {...field} disabled={deleteDisabledWhenLinked} /></FormControl><FormMessage /></FormItem>)} />
                 )}
            ...
            {isMobile ? (
              <div className="grid grid-cols-3 gap-2 w-full [&_button]:h-10">
                ...
                <Button type="submit" disabled={isLoading || editingDisabled} className="w-full">
                  {isLoading ? "..." : (voucher ? "Update" : "Save")} Receipt
                </Button>
