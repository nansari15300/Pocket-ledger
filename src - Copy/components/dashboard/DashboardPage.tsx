...
export default function DashboardPage() {
  ...
  const financialSummary = React.useMemo(() => {
    ...
    const addToGroup = (entity: any, type: 'party' | 'staff' | 'tax') => {
        ...
        filteredVouchers.forEach(v => {
            ...
            } else if (v.lineItems?.some((li: any) => li.taxAccountId === entity.id) && v.lineItems.find((li:any) => li.taxAccountId === entity.id).taxAmount) {
                const taxAmount = v.lineItems.find((li:any) => li.taxAccountId === entity.id).taxAmount;
  ...
  const handlePrint = () => {
    ...
    const excludeOpeningBalance = (arr: { party: string }[]) => arr.filter(p => p.party !== "Opening Balance");
        ...
        {can("view_receivable_payable_summary") && (
            <Card className="col-span-1 transition-colors border-foreground/20">
                ...
                <CardContent className="p-4 pt-0 space-y-2">
                    <div className="flex items-baseline justify-between"><span className="text-xs text-muted-foreground">To Receive</span><span className="text-base font-bold text-green-600">{formatCurrency(financialSummary.totalReceivable, {noSuffix: true})} <span className="text-xs">Dr</span></span></div>
                    <div className="flex items-baseline justify-between"><span className="text-xs text-muted-foreground">To Pay</span><span className="text-base font-bold text-red-600">{formatCurrency(Math.abs(financialSummary.totalPayable), {noSuffix: true})} <span className="text-xs">Cr</span></span></div>
                    <div className="flex items-baseline justify-between pt-2 mt-2 border-t"><span className="text-sm font-bold">Net</span><span className={cn('text-lg font-bold', netBalance >= 0 ? "text-green-600" : "text-red-600")}>{formatCurrency(netBalance, { showDrCr: true })}</span></div>
                    {reportsEnabled && (
                        <div className="text-right pt-2">
                            <Dialog open={receivablesPayablesOpen} onOpenChange={setReceivablesPayablesOpen}>
