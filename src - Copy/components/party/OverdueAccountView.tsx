...
export function OverdueAccountView({
  ...
  const filteredRows = useMemo(() => {
    ...
    if (userQ) {
      list = list.filter((t) => {
        const name = t.userId ? (userNames[t.userId] ?? "").toLowerCase() : "";
          ...
          <div className="flex items-center justify-between gap-4 flex-wrap">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <span className={cn("font-semibold", totalDebit - totalCredit >= 0 ? "text-green-600" : "text-red-600")}>
                {formatCurrency(totalDebit - totalCredit, { showDrCr: true, noAnimation: true })}
              </span>
          ...
          <Table className="table-auto w-full min-w-0">
            ...
            <TableBody>
              ...
              {paginatedRows.map((t) => {
                ...
                const displayUserName = t.userId ? (userNames[t.userId] ?? "—") : "—";
                return (
