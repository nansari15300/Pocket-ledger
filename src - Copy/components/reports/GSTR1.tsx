        ...
        <CardHeader className="flex-shrink-0">
          <div className="flex justify-between items-center">
            <div>
              ...
              <CardDescription>Summary of all outward supplies (sales).</CardDescription>
            </div>
              ...
              <TableBody>
                {gstr1Data.map((row, idx) => (
                  <TableRow key={idx}>
                    ...
                    <TableCell className="text-right">{formatCurrency(row.taxableAmount, { noAnimation: true })}</TableCell>
                    <TableCell className="text-right">{formatCurrency(row.taxAmount, { noAnimation: true })}</TableCell>
                    <TableCell className="text-right">{formatCurrency(row.totalAmount, { noAnimation: true })}</TableCell>
                  </TableRow>
                ))}
                {gstr1Data.length === 0 && (
                  <TableRow>
              ...
              <Table>
                <TableBody>
                  <TableRow className="font-bold">
                    ...
                    <TableCell className="text-right">{formatCurrency(totals.taxableAmount, { noAnimation: true })}</TableCell>
                    <TableCell className="text-right">{formatCurrency(totals.taxAmount, { noAnimation: true })}</TableCell>
                    <TableCell className="text-right">{formatCurrency(totals.totalAmount, { noAnimation: true })}</TableCell>
                  </TableRow>
                </TableBody>
              </Table>
            </div>
