...
export function usePageMemory<T extends { id: string }>(
    ...
    if (!isLoading && !isInitialized.current && currentItems.length > 0) {
      ...
      if (savedState) {
        try {
          ...
          if (parsed.activeView && parsed.activeView !== activeView) {
  ...
  }, [isLoading, storageKey, activeView, setActiveView]); // Removed currentItems to be safe
  ...
  }, [activeView, isLoading, currentItems, disableAutoSelect, urlSelectedId]); 
