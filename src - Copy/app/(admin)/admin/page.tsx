            ...
            <CardContent className="h-[36rem] min-h-[420px] flex flex-col p-4 pb-[4px]">
                 ...
                 <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={chartData} margin={{ top: 8, right: 16, bottom: 0, left: 0 }} barCategoryGap="10%">
                        ...
                        <XAxis dataKey="name" interval={0} height={1} tickLine={false} axisLine={{ stroke: '#e2e8f0' }} tick={(props: { x?: number; y?: number; payload?: { value?: string } }) => {
                          const { x = 0, y = 0, payload } = props;
                          const ty = (y ?? 0) - 88;
                          return payload?.value ? (
                            <text x={x} y={ty} textAnchor="middle" fill="black" fontSize={24} fontWeight={500} transform={`rotate(-90, ${x}, ${ty})`} style={{ overflow: 'visible' }}>{payload.value}</text>
                          ) : null;
                        }} />
                        <YAxis />
