...
export async function initializeCompanyData(companyId: string, userId: string) {
  ...
  const accountsToCreate = [
    ...
    // Salary Account for Add Salary debit account
  ...
  // --- 4. Update company document with default voucher settings ---
  // Skip if company doc doesn't exist yet (e.g. client's setDoc was queued while offline).
  // The sync effect will retry when the doc is created.
...
export async function sendSecurityAlert(
  ...
  try {
    ...
    const liveAlertMessage = `🚨 Security Alert: User "${attemptedByEmail}" tried to restore your company "${companyName}" on ${attemptDate}. But you don't need to worry, we blocked the attempt automatically. This protective measure is active to prevent anyone from restoring another's company illegally.`;
