...
export async function saveVoucher(
  ...
  if (!voucherRef) {
    const companySnap = await getDoc(doc(firestore, "companies", companyId));
    const planId = (companySnap.data()?.planId as PlanId) || "basic";
    ...
    const docRef = await addDoc(collection(firestore, voucherPath), {
      ...
      createdAt: serverTimestamp(),
      history: [
        { changedAt: new Date(), changedBy: userId, changes: { created: { from: "N/A", to: "Created" } } },
      ],
    });
  ...
  const newEntry = { changedAt: new Date(), changedBy: userId, changes: changedFields };
