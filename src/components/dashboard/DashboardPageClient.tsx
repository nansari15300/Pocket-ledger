...
export function DashboardPageContent() {
  ...
  const renderDashboardContent = () => {
    ...
    const showBank = shouldShow("bank-cash-summary");
    const showDaybook = shouldShow("daybook");
    const showRecent = shouldShow("recent-transactions");
    if (showFinancial) dashboardKeepAlivePanelsRef.current.add("financial-summaries");
    if (showCharts) dashboardKeepAlivePanelsRef.current.add("dashboard-charts");
    if (showBank) dashboardKeepAlivePanelsRef.current.add("bank-cash-summary");
    if (showDaybook) dashboardKeepAlivePanelsRef.current.add("daybook");
    if (showRecent) dashboardKeepAlivePanelsRef.current.add("recent-transactions");
    const keep = dashboardKeepAlivePanelsRef.current;
    return (
    <div className="space-y-[5px]">
      {/* Keep consistent 5px spacing between dashboard cards/sections. */}
      {keep.has("financial-summaries") && (
        <div className={cn(!showFinancial && "hidden")} inert={!showFinancial || undefined}>
          {renderFinancialSummaries(isReportsEnabled, false)}
        </div>
      )}
      {keep.has("dashboard-charts") && (
        <div className={cn(!showCharts && "hidden")} inert={!showCharts || undefined}>
          {renderFinancialSummaries(isReportsEnabled, true)}
        </div>
      )}
      {can('view_bank_cash_summary') && keep.has("bank-cash-summary") && (
        <div className={cn(!showBank && "hidden")} inert={!showBank || undefined}>
          <BankCashSummary />
        </div>
      )}
      ...
      {can('view_recent_transactions') && keep.has("recent-transactions") && (
        <div className={cn(!showRecent && "hidden")} inert={!showRecent || undefined}>
