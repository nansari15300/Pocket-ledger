...
export function CreateStaffGroupDialog({
  onGroupCreated,
  children,
  isOpen,
  onOpenChange,
  ...
  const resetForm = () => {
    setChainSlots([{}]);
    setSystemBranch(STAFF_ENTITY_GROUP_PRESET.defaultBranch);
  };
  ...
  useEffect(() => {
    ...
    document.addEventListener(
      STAFF_ENTITY_GROUP_PRESET.prefillEventName,
  ...
  async function handleSubmit(saveAndNew: boolean = false) {
    ...
    for (const name of masterEntityGroupCreateChainPendingNames(chainSlots)) {
      if (isSystemGroupName("staff", name)) {
        toast({
          variant: "destructive",
