...
export function EditStaffGroupDialog({ group, allGroups, onGroupUpdated, onGroupDeleted, children, isOpen, onOpenChange, hasAccounts }: {
  group: StaffGroup;
  ...
  /** APK cloud company offline: staff group edit Save/Bin voucher parity. */
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      name: group.name,
      parentId: group.parentId || "loans_liabilities",
    },
  });
  ...
  function onSubmit(values: z.infer<typeof formSchema>): void {
    ...
    const gid = group.id;
    setDialogOpen(false); // Immediate close — same UX as Edit Party / Item
    void (async () => {
      setIsLoading(true);
      try {
  ...
  const handleDelete = async () => {
    ...
    if (hasAccounts) {
      sonnerToast.error("Cannot Delete", { description: "This group has staff members and cannot be deleted." });
    ...
    try {
        ...
        onGroupDeleted();
        setDialogOpen(false); // Only close the main dialog
    } catch (error) {
                    ...
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent>
                         <SelectGroup>
                              ...
                              <SelectItem value="loans_liabilities">Loans &amp; Liabilities</SelectItem>
