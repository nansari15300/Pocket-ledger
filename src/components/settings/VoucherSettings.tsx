...
const voucherSettingsSchema = z.object({
  ...
  /** Voucher edit history: on/off, max entries, and behavior when full. */
...
export function VoucherSettings() {
  ...
  /** Plan cap from super admin — company cannot exceed this. */
  const [planHistoryLimit, setPlanHistoryLimit] = useState<number>(100);
  ...
  // Fetch plan's max history limit so we cap company setting
  useEffect(() => {
    if (!companyId) return;
  ...
  async function onSubmit(data: VoucherSettingsValues): Promise<void> {
    ...
    setIsLoading(true);
    try {
      // Cap voucherHistoryLimit to plan's max (super admin setting)
      ...
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            ...
            {/* Voucher History: On/Off, max entries, behavior when full */}
            {isCompanyOwner && (
              <div className="space-y-4">
