...
function getInitialFormValues(voucher?: any): ProductionFormValues {
  ...
  return {
    ...
    finishedGoods: voucher.finishedGoods?.map((fg: any) => ({ ...fg, hsCode: fg.hsCode || "" })) || [{ itemId: "", quantity: 1, unit: "", rate: 0, amount: 0, hsCode: "" }],
          ...
          <div className="space-y-4 min-w-0 max-w-full w-full overflow-x-hidden [&>*]:min-w-0 [&>*]:max-w-full">
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <Button type="button" variant="outline" size="sm" onClick={() => appendFinishedGood({ itemId: "", quantity: 1, unit: "", rate: 0, amount: 0, hsCode: "" })}>
                  <PlusCircle className="h-4 w-4 mr-1" /> Add
                </Button>
              </div>
              <div className="space-y-2 border rounded-md p-2">
                  ...
                  <div key={field.id} className="grid grid-cols-12 gap-2 items-end">
                      ...
                      <Combobox
                        ...
                        onChange={(value, newName) => {
                          if (value === "add-new") {
                            ...
                            if (newName) {
                              setTimeout(() => {
                                document.dispatchEvent(new CustomEvent('prefill-create-item-name', { detail: { name: newName, type: 'finished_good' } }));
                              }, 100);
                            }
                          } else {
                    ...
                    <div className="col-span-1">
                      <Button 
                        ...
                        size="icon" 
                        onClick={() => {
                          if (finishedGoodFields.length > 1) {
                            removeFinishedGood(index);
                          } else {
                            toast({
                              variant: "destructive",
                              title: "Cannot Delete",
                              description: "At least one finished good is required.",
                            });
                          }
                        }}
                      >
        ...
        <div className={cn("border-t min-w-0 max-w-full overflow-x-hidden", isMobile ? "mt-[3px] pt-[3px] pb-[3px] px-2" : "p-4 flex justify-between items-center")}>
          {isMobile ? (
            <div className="grid grid-cols-3 gap-2 w-full min-w-0 [&_button]:h-10 [&_button]:rounded-full [&_button:disabled]:opacity-45 [&_button:disabled]:shadow-[inset_0_4px_12px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(0,0,0,0.25)] [&_button:disabled]:brightness-50 [&_button:disabled]:saturate-50 [&_button:disabled]:scale-[0.98] [&_button:disabled]:cursor-not-allowed [&_button:disabled]:text-opacity-[0.70]">
              ...
              <Button type="button" onClick={async (e) => { e.preventDefault(); if (isFormDirty) { approveAfterSaveRef.current = true; form.handleSubmit(onSubmit)(); } else onApprove?.(); }} disabled={!showApproveButton || !onApprove || isApproving} className="w-full bg-emerald-700 hover:bg-emerald-800 text-white border-0 hover:text-white">
            ...
            <>
                ...
                {isEditing && (
                  <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
                    <AlertDialogTrigger asChild disabled={editingDisabled || deleteDisabledWhenLinked}>
                      <Button type="button" variant="destructive" size="sm" disabled={editingDisabled || deleteDisabledWhenLinked}>
                        Delete
                      </Button>
                    </AlertDialogTrigger>
                    <AlertDialogContent>
              ...
              <div className="flex gap-2 [&_button]:h-10">
                ...
                {showApproveButton && onApprove && (
                  <Button type="button" variant="default" onClick={async (e) => { e.preventDefault(); if (isFormDirty) { approveAfterSaveRef.current = true; form.handleSubmit(onSubmit)(); } else onApprove(); }} disabled={isApproving} className="shrink-0">
