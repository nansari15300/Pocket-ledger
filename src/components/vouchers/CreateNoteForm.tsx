...
export function CreateNoteForm({ 
  ...
  async function processAndSave(values: NoteFormValues, saveAndNew: boolean = false, onSuccess?: () => void, approveAfterSave?: boolean) {
    ...
    try {
        ...
        if (approveAfterSave && result?.id && !voucher?.id) {
          ...
          sonnerToast.success("Note saved and approved.", { id: toastId });
        } else {
  ...
  return (
    <Form {...form}>
      <form onSubmit={(e) => handleFormSubmit(e)} className="h-full flex flex-col min-w-0 w-full max-w-full">
        ...
        <div className={cn("border-t min-w-0 max-w-full overflow-x-hidden", isMobile ? "mt-[3px] pt-[3px] pb-[3px]" : "pt-4 flex flex-col md:flex-row justify-end gap-2 md:gap-4")}>
            {isMobile ? (
              <div className="grid grid-cols-3 gap-2 w-full min-w-0 [&_button]:h-10 [&_button]:rounded-full [&_button:disabled]:opacity-45 [&_button:disabled]:shadow-[inset_0_4px_12px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(0,0,0,0.25)] [&_button:disabled]:brightness-50 [&_button:disabled]:saturate-50 [&_button:disabled]:scale-[0.98] [&_button:disabled]:cursor-not-allowed [&_button:disabled]:text-opacity-[0.70]">
                ...
                <Button type="button" className="w-full bg-sky-600 hover:bg-sky-700 text-white border-0 opacity-60" disabled>
                  History
                </Button>
                <Button type="button" className="w-full bg-amber-600 hover:bg-amber-700 text-white border-0 opacity-60" disabled>
                  Save & Print
                </Button>
                {/* Row 1: Cancel (left) | Approve or Save & Approve (middle, when can approve) | Save (right, always) */}
                <Button type="button" onClick={() => onVoucherAction?.('cancelled')} className="w-full bg-pink-300 hover:bg-pink-400 text-pink-950 border-0">Cancel</Button>
                {voucher?.id ? (
                  <Button type="button" onClick={async (e) => { e.preventDefault(); if (isFormDirty) await handleFormSubmit(e, { approveAfterSave: true }); else onApprove?.(); }} disabled={!showApproveButton || !onApprove || isApproving} className="w-full bg-emerald-700 hover:bg-emerald-800 text-white border-0 hover:text-white">{isApproving ? "..." : isFormDirty ? "Save & Approve" : "Approve"}</Button>
                ) : showSaveAndApproveOnCreate ? (
                  <Button type="button" onClick={(e) => handleFormSubmit(e, { approveAfterSave: true })} disabled={isLoading || editingDisabled} className="w-full bg-emerald-700 hover:bg-emerald-800 text-white border-0 hover:text-white">{isLoading ? "..." : "Save & Approve"}</Button>
                ) : (
                  <Button type="button" disabled className="w-full bg-muted text-muted-foreground border-0 opacity-50">—</Button>
                )}
                <Button type="submit" disabled={isLoading || editingDisabled} className="w-full bg-green-200 hover:bg-green-300 text-green-900 dark:bg-green-800/60 dark:hover:bg-green-700/60 dark:text-green-100 border-0">{isLoading ? "..." : "Save"}</Button>
              </div>
            ) : (
              <>
                <Button type="button" variant="outline" onClick={() => onVoucherAction?.('cancelled')} className="w-full md:w-auto h-10">Cancel</Button>
                <Button type="submit" disabled={isLoading || editingDisabled} className="w-full md:w-auto h-10">{isLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Save</Button>
                {showSaveAndApproveOnCreate && (
                  <Button type="button" onClick={(e) => handleFormSubmit(e, { approveAfterSave: true })} disabled={isLoading || editingDisabled} className="w-full md:w-auto h-10">{isLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Save & Approve</Button>
                )}
                {showApproveButton && onApprove && (
                  <Button type="button" variant="default" onClick={async (e) => { e.preventDefault(); if (isFormDirty) await handleFormSubmit(e, { approveAfterSave: true }); else onApprove(); }} disabled={isApproving} className="w-full md:w-auto shrink-0 h-10">
