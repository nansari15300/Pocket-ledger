...
const { isDirty: _isFormFieldsDirty } = form.formState;
  ...
  useEffect(() => {
    ...
    // Ledger clear tabhi jab expense account list hydrate ho.
    if (eid && expenseAccounts.length > 0 && !expenseAccounts.some((e: any) => e.id === eid)) {
    ...
    // "Other" ledger clear tabhi jab expense account list hydrate ho.
    if (payeeType === "other" && toAcc && expenseAccounts.length > 0 && !expenseAccounts.some((e: any) => e.id === toAcc)) {
  ...
  useEffect(() => {
    ...
    if (cur.length > incoming.length || (cur.length > 0 && incoming.length === 0)) {
      void import("@/lib/attachmentDeleteTrace").then((m) =>
        m.logAttachWipe({
          source: "CreatePaymentOutForm.voucherFileUrlsEffect",
      ...
      if (savedDoc && savedDoc.id) {
        ...
        const postSaveTail = async () => {
          ...
          try {
            ...
            if (docId && companyId) {
              const initialUrls = initialFilesRef.current || [];
              const savedUrls = rawFileUrlsForPostSave;
              const attachmentsChanged =
                _isFileDirty ||
                savedUrls.length !== initialUrls.length ||
                savedUrls.some((u, i) => u !== initialUrls[i]);
              if (attachmentsChanged || savedUrls.length > 0) {
