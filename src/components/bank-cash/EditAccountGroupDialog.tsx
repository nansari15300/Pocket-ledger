...
export function EditAccountGroupDialog({ group, allGroups, onGroupUpdated, onGroupDeleted, children, hasAccounts }: {
  ...
  useEffect(() => {
    if (isOpen) {
      const decoded = decodeMasterEntityGroupParentPath(
        liveGroup,
        allGroups,
        BANK_ENTITY_GROUP_PRESET.config
      );
  ...
  const handleAddNewParentAtLevel = useCallback(
    async (levelIndex: number, name: string) => {
      ...
      if (isSystemGroupName("account", name.trim())) {
        toast({
          variant: "destructive",
      ...
      try {
        const newId = await createOneMasterEntityGroup({
          ...
          name: name.trim(),
          parentId,
          collection: BANK_ENTITY_GROUP_PRESET.collection,
          localIdPrefix: BANK_ENTITY_GROUP_PRESET.localIdPrefix,
        });
  ...
  function handleSubmit(): void {
    ...
    if (isSystemGroupName("account", editedName)) {
      toast({
        variant: "destructive",
        title: "System Group Name",
    ...
    void (async () => {
      setIsLoading(true);
      try {
        const groupRef = doc(firestore, `companies/${companyId}/account_groups`, gid);
  ...
  const handleMoveToBin = async () => {
    ...
    try {
      const res = await softDeleteCompanySubdocToRecycleBin(
        companyId,
        BANK_ENTITY_GROUP_PRESET.collection,
        group.id,
        user.uid
      );
      if (!res.ok) throw new Error("error" in res ? res.error : "Move to bin failed");
      ...
      toast({ variant: "destructive", title: "Delete Failed", description: "An error occurred while moving the group to the recycle bin." });
    } finally {
      setIsLoading(false);
      setIsDeleteDialogOpen(false);
    }
  ...
  const handlePermanentDelete = async () => {
    ...
    setIsLoading(true);
    try {
      await permanentDeleteCompanySubdocFromRecycleBin(companyId, BANK_ENTITY_GROUP_PRESET.collection, group.id);
      toast({ title: "Group Deleted Permanently", description: `"${group.name}" has been permanently removed.` });
      onGroupDeleted();
      setIsOpen(false);
    } catch (error) {
      console.error("Error permanently deleting group:", error);
      toast({ variant: "destructive", title: "Delete Failed", description: "An error occurred while permanently deleting the group." });
    } finally {
      setIsLoading(false);
      setIsDeleteDialogOpen(false);
    }
