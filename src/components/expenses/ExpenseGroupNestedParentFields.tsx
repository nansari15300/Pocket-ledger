...
import {
  EXPENSE_GROUP_STEPPED_DIRECT_VALUE,
  EXPENSE_GROUP_TOP_PARENT_OPTIONS,
  expenseGroupPendingSlotValue,
  isExpenseGroupCreateChainSlotEmpty,
  listExpenseGroupChildrenForParent,
  listExpenseGroupLevelOptions,
  parseExpenseGroupPendingSlotValue,
  type ExpenseGroupCreateChainSlot,
} from "@/lib/expenseGroupTree";

export const EXPENSE_GROUP_NESTED_FORM_INDENT_PX = 15;

export type ExpenseGroupSteppedParentState = {
  systemBranch: ExpenseGroupListBranch;
  parentPathIds: string[];
};

type ExpenseGroupNestedParentFieldsProps = {
...
function getCreateSlot(slots: ExpenseGroupCreateChainSlot[], index: number): ExpenseGroupCreateChainSlot {
  return slots[index] || {};
}
...
function slotToComboboxValue(
  slot: ExpenseGroupCreateChainSlot,
  levelIndex: number
): string {
  if (slot.groupId) return slot.groupId;
  const pending = String(slot.pendingName || "").trim();
  if (pending) return expenseGroupPendingSlotValue(pending);
  return "";
}
...
function normalizeCreateChainLength(slots: ExpenseGroupCreateChainSlot[]): ExpenseGroupCreateChainSlot[] {
  if (slots.length === 0) return [{}];
  return slots;
}

export function ExpenseGroupNestedParentFields({
  allGroups,
  excludeGroupId,
  systemBranch,
  ...
  const handleBranchChange = (value: string) => {
    const branch = value === "income" ? "income" : "expenses";
    onSystemBranchChange(branch);
    if (isCreate) {
      onChainSlotsChange?.([{}]);
      setSlotCount(1);
      return;
  ...
  const setCreateSlots = (slots: ExpenseGroupCreateChainSlot[]) => {
  ...
  const handleCreateLevelChange = (levelIndex: number, value: string, newName?: string) => {
    ...
    const pendingFromValue = parseExpenseGroupPendingSlotValue(value);
    if (pendingFromValue) {
    ...
    if (levelIndex === 0 && value === EXPENSE_GROUP_STEPPED_DIRECT_VALUE) {
      slots[0] = {};
      setCreateSlots([{}]);
      return;
    }
  ...
  const renderCreateLevelSlot = (levelIndex: number) => {
    ...
    const levelDisabled =
      disabled ||
      (levelIndex > 0 && isExpenseGroupCreateChainSlotEmpty(getCreateSlot(chainSlots, levelIndex - 1)));
    ...
    let comboboxOptions =
      levelIndex === 0
        ? [
            ...(pendingName
              ? [{ value: expenseGroupPendingSlotValue(pendingName), label: pendingName }]
              : []),
    ...
    const canRemove =
      slotCount > 1 && (levelIndex > 0 || !isExpenseGroupCreateChainSlotEmpty(slot));
  ...
  const renderLevelSlot = (levelIndex: number) => {
    ...
    const options =
      levelIndex === 0
        ? listExpenseGroupLevelOptions(allGroups, systemBranch, parentPathIds, 0, excludeGroupId)
        : parentForLevel
          ? listExpenseGroupChildrenForParent(allGroups, String(parentForLevel), excludeGroupId)
          : [];
    ...
    <div className="space-y-3">
      <div className="space-y-1.5">
        ...
        <Select value={systemBranch} onValueChange={handleBranchChange} disabled={disabled}>
          <SelectTrigger>
            <SelectValue placeholder="Select Income or Expenses" />
          </SelectTrigger>
          <SelectContent>
            {EXPENSE_GROUP_TOP_PARENT_OPTIONS.map((p) => (
