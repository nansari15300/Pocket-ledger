...
export function EditExpenseGroupDialog({ group, allGroups, onGroupUpdated, onGroupDeleted, children, hasAccounts }: {
  ...
  const canDeleteGroup = !hasAccounts && !hasDirectChildGroups && !apkOfflineViewOnly;

  useEffect(() => {
    if (isOpen) {
      const decoded = decodeExpenseGroupParentPath(liveGroup, allGroups);
  ...
  const handleAddNewParentAtLevel = useCallback(
    async (levelIndex: number, name: string) => {
      ...
      if (isSystemGroupName("expense", name.trim())) {
        toast({
          variant: "destructive",
  ...
  async function handleSubmit() {
    ...
    if (isSystemGroupName("expense", editedName)) {
      toast({
        variant: "destructive",
        title: "System Group Name",
    ...
    const resolvedParentId = resolveExpenseGroupParentIdFromPath(systemBranch, parentPathIds);
    ...
    try {
      ...
      toast({
        title: "Group Updated!",
        description: `"${editedName}" has been successfully updated.`,
      });
      onGroupUpdated();
      setIsOpen(false);
    } catch (error) {
  ...
  const handlePermanentDelete = async () => {
    ...
    setIsLoading(true);
    try {
      await permanentDeleteCompanySubdocFromRecycleBin(companyId, "expense_groups", group.id);
      toast({
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              System group → Users Parent group → Child group. Add Child group se unlimited tree depth (15px indent).
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <NestedExpenseGroupFields
              ...
              onAddNewParentAtLevel={(level, name) => void handleAddNewParentAtLevel(level, name)}
              groupName={groupName}
