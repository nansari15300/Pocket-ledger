...
export async function POST(req: NextRequest) {
  ...
  const body = (await req.json()) as { remotePath?: string; driveSharedFolderId?: string };
  const remotePath = String(body.remotePath || "").trim();
  if (!remotePath) return NextResponse.json({ error: "remotePath required" }, { status: 400 });
  try {
    // Shared-folder cleanup keeps force re-encrypt from deleting/checking a wrong My Drive clone.
    await driveDeleteFileByRemotePath(
      auth.uid,
      remotePath,
