...
/**
 * SuperAdmin check via Admin SDK (matches app + avoids client rules quirks: slug user docs, superAdminEmails).
 */
async function isSuperAdminServer(uid: string, email: string | undefined): Promise<boolean> {
  const db = getAdminDb();
