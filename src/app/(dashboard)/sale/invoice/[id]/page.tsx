...
export default function InvoicePage() {
      ...
      async (docSnap) => {
        if (docSnap.exists()) {
          ...
          if (invoiceData.partyId) {
            ...
            if (partySnap.exists()) {
              associatedParty = partySnap.data() as Party;
            }
          }
          ...
          if (invoiceData.lineItems && invoiceData.lineItems.length > 0) {
            for (const item of invoiceData.lineItems) {
              if (item.itemId) {
                ...
                if (itemSnap.exists()) {
                  newItemsData[item.itemId] = itemSnap.data();
                }
              }
            }
          }
  ...
  if (loading) {
    return (
      <div className="flex h-screen w-screen flex-col gap-4 items-center justify-center bg-gray-50 text-black">
  ...
  if (!invoice) {
    return (
      <div className="flex h-screen w-screen flex-col gap-4 items-center justify-center bg-gray-50 text-black">
        <p className="text-lg font-medium text-red-500">Invoice not found.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-gray-50 py-4">
      ...
      <div
        ref={invoiceRef}
        className="w-[210mm] h-[297mm] bg-white text-black mx-auto border border-black font-sans text-sm p-4 flex flex-col"
      >
        {/* Header */}
        <div className="flex items-center justify-between pb-2 border-b border-black">
          <div className="w-1/4">
            <Image
              src="https://picsum.photos/seed/bpslogo/150/50"
              alt="Company Logo"
              width={150}
              height={50}
            />
          ...
          <div className="w-1/4 text-right">
            <h2 className="text-lg font-semibold underline">Tax Invoice</h2>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-x-4 mt-2 pb-2 text-xs border-b border-black">
          <div>
            <p>Customer Name : {party?.name || "N/A"}</p>
            <p>Pan / Vat No. : {party?.pan || "N/A"}</p>
            <p>Customer Address : {party?.address || "N/A"}</p>
            <p>Customer Cnt No. : {party?.phone || "N/A"}</p>
          </div>
          <div>
            <p>Invoice No. : {invoice.voucherNumber}</p>
            <p>Date of Transaction : {formatDate(invoiceDate)}</p>
            <p>Miti of Transaction : {formatDateBS(invoiceDate)}</p>
          </div>
        </div>
        ...
        <div className="flex justify-between mt-1 pb-1 text-xs border-b border-black">
          ...
          <p>
            <span className="font-semibold">Bill Type :</span> Credit
          </p>
        </div>
        ...
        <table className="w-full text-xs border-collapse mt-2 table-fixed border-l border-r border-black">
          ...
          <tbody>
            {invoice.lineItems.map((item: any, index: number) => (
              <tr key={index}>
                ...
                <td className="border border-black text-center">
                  {itemsData[item.itemId]?.hsCode || ""}
                </td>
                <td className="border border-black px-1">
                  {itemsData[item.itemId]?.name || "Item Name"}
                </td>
                <td className="border border-black text-center">
            ...
            {Array.from({
              length: Math.max(0, TOTAL_ROWS - invoice.lineItems.length),
            }).map((_, i) => (
           ...
           <tfoot className="border-t border-black">
              <tr>
                  <td rowSpan={5} colSpan={3} className="border-r border-black p-1 align-top">
                      <div className="flex flex-col h-full">
                          <div className="border-b border-black p-1 flex-grow">
                            <span className="font-semibold">Remarks:</span> {invoice.narration || 'N/A'}
                          </div>
                           <div className="p-1 flex-grow">
                             <span className="font-semibold not-italic">In Words:</span>{' '}
                             <span className="italic">Rs. {numToWords(invoice.total)} Only</span>
                          </div>
                      </div>
                  </td>
              ...
              <tr>
                  <td colSpan={4} className="text-right p-1 font-medium border-x border-black">Discount</td>
                  <td className="text-right p-1 border-r border-black">{invoice.discount.toFixed(2)}</td>
              </tr>
              <tr>
                  <td colSpan={4} className="text-right p-1 font-medium border-x border-black">Taxable Value</td>
                  <td className="text-right p-1 border-r border-black">{taxableValue.toFixed(2)}</td>
              </tr>
              <tr>
                  <td colSpan={4} className="text-right p-1 font-medium border-x border-black">VAT 13%</td>
                  <td className="text-right p-1 border-r border-black">{invoice.tax.toFixed(2)}</td>
              </tr>
              <tr className="font-bold border-b border-black">
                  <td colSpan={4} className="text-right p-1 border-t border-x border-black">Net Amount</td>
                  <td className="text-right p-1 border-t border-r border-black">{invoice.total.toFixed(2)}</td>
              </tr>
            </tfoot>
        </table>
        </div>
        ...
        <div className="mt-auto pt-4 text-xs">
          <div className="flex justify-between items-end">
              <div className="text-center">
                  <p className="border-t border-dotted border-black w-32 pt-1">
                  Received By
                  </p>
              </div>
              <div className="text-center">
                  <p className="font-semibold">ISHWOR</p>
                  <p className="border-t border-dotted border-black w-32 pt-1">
                  Prepared By
                  </p>
              </div>
              <div className="text-center">
                  <p className="border-t border-dotted border-black w-40 pt-1">
                    For : {company?.name}
                  </p>
              </div>
          </div>
          <p className="text-right mt-2">
            Print Date & Time :{" "}
            {formatDateFns(new Date(), "dd/MM/yyyy h:mm:ss a")}
          </p>
        </div>
      </div>
    </div>
