...
export function useDeviceLimit() {
  ...
  useEffect(() => {
    ...
    const runCheck = () => {
      if (cancelled) return;
      registerDeviceAndCheckLimit(companyId, user!.uid, maxDevices, true)
        .then((r) => {
  ...
  useEffect(() => {
    ...
    getOrCreateDeviceId().then((id) => {
      deviceId = id;
      const cmdRef = doc(firestore, "companies", companyId, "device_commands", id);
      return onSnapshot(cmdRef, (snap) => {
