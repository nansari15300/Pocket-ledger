...
import { registerDeviceAndCheckLimit, replaceMyOtherDevicesAndRegister, getOrCreateDeviceId, setKickedForCompany, clearKickedForCompany } from "@/lib/deviceLimitClient";
...
export function useDeviceLimit() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const wasKicked = typeof window !== "undefined" && window.localStorage.getItem("pl_kicked_" + companyId) === "1";
    const runCheck = () => {
      if (cancelled) return;
      const nowKicked = typeof window !== "undefined" && window.localStorage.getItem("pl_kicked_" + companyId) === "1";
      registerDeviceAndCheckLimit(companyId, user!.uid, maxDevices, true, { userCanUseMultiDevice, isOwner, wasKicked: nowKicked })
        .then((r) => {
