...
import { registerDeviceAndCheckLimit, replaceMyOtherDevicesAndRegister, getOrCreateDeviceId, setKickedForCompany, clearKickedForCompany, getWasKicked } from "@/lib/deviceLimitClient";
...
export function useDeviceLimit() {
  ...
  useEffect(() => {
    ...
    const runCheck = () => {
      ...
      registerDeviceAndCheckLimit(companyId, user!.uid, maxDevices, true, { userCanUseMultiDevice, isOwner, wasKicked })
        .then((r) => {
          if (!cancelled) {
            const inList = r.allowed || (r.kickedAndBlocked && wasKicked);
            if (myDeviceWasInListRef.current === null) myDeviceWasInListRef.current = inList;
            setResult({ allowed: r.allowed, count: r.count, limit: r.limit, singleDeviceOnly: r.singleDeviceOnly, replaceOffer: r.replaceOffer, noPermissionNewDevice: r.noPermissionNewDevice, kickedAndBlocked: r.kickedAndBlocked });
          }
        })
        .catch(() => {
  ...
  const clearKickedAndRefresh = useCallback(() => {
    if (!companyId) return;
