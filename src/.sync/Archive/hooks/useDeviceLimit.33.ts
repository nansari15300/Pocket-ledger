...
export function useDeviceLimit() {
  ...
  const inListRef = useRef<boolean>(false);
  ...
  useEffect(() => {
    ...
    const runCheck = (): Promise<void> => {
      ...
      return registerDeviceAndCheckLimit(companyId, user!.uid, maxDevices, true, { userCanUseMultiDevice, isOwner, wasKicked })
        .then(async (r) => {
          ...
          if (!cancelled) {
            const useListRule = snapshotReceivedRef.current;
            const inList = inListRef.current;
            const allowed = useListRule && !inList ? false : r.allowed;
            const kickedAndBlocked = useListRule && !inList ? true : r.kickedAndBlocked;
            setResult({ allowed, count: r.count, limit: r.limit, singleDeviceOnly: r.singleDeviceOnly, replaceOffer: r.replaceOffer, noPermissionNewDevice: r.noPermissionNewDevice, kickedAndBlocked });
          }
        })
        .catch(() => {
    ...
    const unsubDevices = onSnapshot(devicesRef, (snap) => {
      ...
      if (myDeviceWasInListRef.current === true && !inList) {
        ...
        setResult((prev) => (prev ? { ...prev, allowed: false, kickedAndBlocked: true } : null));
      }
  ...
  const clearKickedAndRefresh = useCallback((): Promise<void> => {
    ...
    return typeof p?.then === "function" ? p : Promise.resolve();
  }, [companyId]);
