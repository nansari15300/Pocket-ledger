...
import { getOrCreateDeviceId, removeThisDevice, trimDeviceHistoryToLimit, addDeviceHistoryEntryWhenRemoved, enforceDeviceLimitByPlan } from "@/lib/deviceLimitClient";
...
export function ManageDevices() {
  ...
  useEffect(() => {
    ...
    if (devices.length > maxDevices && maxDevices >= 1) {
      enforceDeviceLimitByPlan(companyId, maxDevices).catch(() => {});
    }
  }, [companyId, company, devices.length, maxDevices]);
