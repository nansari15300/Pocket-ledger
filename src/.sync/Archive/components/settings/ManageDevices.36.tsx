...
import { collection, getDocs, getDoc, deleteDoc, doc, onSnapshot, query, where, updateDoc, orderBy, writeBatch } from "firebase/firestore";
...
import { getOrCreateDeviceId, getDeviceLabel, trimDeviceHistoryToLimit, addDeviceHistoryEntryWhenRemoved, markDeviceKicked } from "@/lib/deviceLimitClient";
...
export function ManageDevices() {
  ...
  const trimmedOnLoadRef = useRef(false);
  const lastAutoTrimCountRef = useRef(0);
  ...
  // Auto-remove devices over plan limit: never remove this device; remove oldest (by lastActive) first.
  useEffect(() => {
    if (!companyId || !isCompanyOwner) return;
    if (devices.length <= maxDevices) {
      lastAutoTrimCountRef.current = 0;
      return;
    }
    if (lastAutoTrimCountRef.current === devices.length) return; // already trimming this count
    lastAutoTrimCountRef.current = devices.length;
    const others = devices.filter((d) => d.id !== currentDeviceId);
    const byOldestFirst = [...others].sort((a, b) => (a.lastActive?.getTime() ?? 0) - (b.lastActive?.getTime() ?? 0));
    const toRemoveCount = devices.length - maxDevices;
    const toRemove = byOldestFirst.slice(0, toRemoveCount);
    ...
    (async () => {
      try {
        ...
        toast({ title: "Devices trimmed", description: `Removed ${toRemove.length} device(s) over plan limit (${maxDevices}). This device was kept.` });
      } catch (e) {
        toast({ title: "Error", description: e instanceof Error ? e.message : "Failed to trim devices", variant: "destructive" });
      }
    })();
  }, [companyId, isCompanyOwner, devices, devices.length, maxDevices, currentDeviceId, toast]);
  ...
  // When history is over limit (e.g. limit was reduced or entries added before we stopped duplicate adds), trim once on load
  useEffect(() => {
    if (!companyId || !isCompanyOwner || deviceHistory.length <= historyLimit || trimmedOnLoadRef.current) return;
    trimmedOnLoadRef.current = true;
    trimDeviceHistoryToLimit(companyId, historyLimit).catch(() => {});
  }, [companyId, isCompanyOwner, historyLimit, deviceHistory.length]);
      ...
      <div className="space-y-8">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Devices that have signed in to this company. View only (admin can manage devices).
            </CardDescription>
          </CardHeader>
          <CardContent>
            <p className="text-sm text-muted-foreground">This device: {getDeviceLabel() || "—"}</p>
          </CardContent>
        </Card>
      </div>
    );
  ...
  return (
    <div className="space-y-8">
                ...
                {sortedDevices.map((device) => {
                  ...
                  <TableRow key={device.id}>
                    <TableCell className="text-sm">
                      {device.deviceLabel || (device.deviceType === "mobile" ? "Mobile" : device.deviceType === "desktop" ? "Desktop" : "—")}
                    </TableCell>
                    ...
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <Button
                          ...
                          disabled={!!kickingId || device.id === currentDeviceId}
                          title={device.id === currentDeviceId ? "Cannot kick out this device" : undefined}
                        >
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Device history
            <span className="text-sm font-normal text-muted-foreground">({deviceHistory.length} entries)</span>
          </CardTitle>
          <CardDescription>
        ...
        <CardContent className="space-y-4">
          <div className="flex flex-wrap items-end gap-3">
            ...
            <span className="text-sm text-muted-foreground pb-2">{deviceHistory.length} entries (keeping up to {historyLimit})</span>
          </div>
            ...
            <Table>
              ...
              <TableBody>
                {deviceHistory.map((row) => {
                  ...
                  const deviceDisplay = row.deviceLabel || (row.deviceType === "mobile" ? "Mobile" : row.deviceType === "desktop" ? "Desktop" : "—");
                  return (
                    <TableRow key={row.id}>
                      <TableCell className="text-sm font-medium">{deviceDisplay}</TableCell>
      ...
      <AlertDialog open={confirmDeleteAllHistory} onOpenChange={(open) => !deletingAllHistory && setConfirmDeleteAllHistory(open)}>
        <AlertDialogContent>
          <AlertDialogTitle>Delete all device history?</AlertDialogTitle>
          <AlertDialogDescription>
            This will remove all {deviceHistory.length} entries. This cannot be undone.
          </AlertDialogDescription>
          <AlertDialogFooter>
