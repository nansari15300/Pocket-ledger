...
export function ManageDevices() {
  ...
  const handleKickOut = async (device: DeviceRow) => {
    ...
    try {
      await addDeviceHistoryEntryWhenRemoved(companyId, { id: device.id, userId: device.userId, deviceType: device.deviceType });
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {deviceHistory.length === 0 ? (
            <p className="text-sm text-muted-foreground py-4">No history yet. Entries are added when a device is kicked/removed from the synced list.</p>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
