...
import { getOrCreateDeviceId, removeThisDevice, trimDeviceHistoryToLimit, addDeviceHistoryEntryWhenRemoved, enforceSingleDevicePerUser } from "@/lib/deviceLimitClient";
...
/** Format date as fixed-width string (MM/DD/YYYY, hh:mm:ss AM) so column does not shift when time updates. */
...
export function ManageDevices() {
  ...
  const handleUserCanUseMultiDeviceChange = async (checked: boolean) => {
    ...
    try {
      ...
      if (!checked) {
        const removed = await enforceSingleDevicePerUser(companyId);
        refreshDeviceCheck();
        toast({
          title: "Setting saved",
          description: removed > 0
            ? `Shared users can use only one device at a time. ${removed} older device(s) removed; each user kept their most recently active device.`
            : "Shared users can use only one device at a time.",
        });
      } else {
        toast({ title: "Setting saved", description: "Shared users can use multiple devices (within plan limit)." });
      }
    } catch (e: unknown) {
  ...
  const handleKickOut = async (device: DeviceRow) => {
    ...
    try {
      ...
      toast({ title: "Device removed", description: "That device will see a message. This device is still in the company." });
      setConfirmKick(null);
      refreshDeviceCheck();
    } catch (e: any) {
  ...
  return (
    <div className="space-y-8">
          ...
          <div className="flex items-center justify-between rounded-lg border p-4">
            <div className="space-y-0.5">
              ...
              <p className="text-sm text-muted-foreground">
                Yes: when the plan has free slots, shared users can sign in on multiple devices (within plan limit). No: each shared user can use only one device at a time; on a new device they must join this device (replace the old one) or log out from the other device.
              </p>
            </div>
            <Switch
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="min-w-[11rem] w-[11rem]">Last active</TableHead>
              ...
              <TableBody>
                {sortedDevices.map((device) => {
                  ...
                  <TableRow key={device.id}>
                    ...
                    <TableCell className="text-sm text-muted-foreground tabular-nums min-w-[11rem] w-[11rem]">
                    ...
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {isThisDevice ? (
                          <span className="text-xs text-muted-foreground">(current device cannot be removed)</span>
                        ) : (
                          <Button
                            variant="ghost"
                            size="sm"
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex flex-wrap items-end gap-3">
            ...
            <span className="inline-flex items-center h-10 px-3 rounded-md border border-input bg-muted/50 text-sm text-muted-foreground shrink-0">{deviceHistory.length} entries (keeping up to {companyHistoryLimit})</span>
          </div>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>User name</TableHead>
                  <TableHead className="min-w-[11rem] w-[11rem]">Last active</TableHead>
                  <TableHead className="min-w-[11rem] w-[11rem]">Recorded at</TableHead>
                  <TableHead>Device type</TableHead>
                  <TableHead className="text-right">Actions</TableHead>
                </TableRow>
              </TableHeader>
                ...
                {deviceHistory.map((row) => {
                  ...
                  return (
                    <TableRow key={row.id}>
                      ...
                      <TableCell className="text-sm text-muted-foreground tabular-nums min-w-[11rem] w-[11rem]">{row.lastActive ? row.lastActive.toLocaleString() : "—"}</TableCell>
                      <TableCell className="text-sm text-muted-foreground tabular-nums min-w-[11rem] w-[11rem]">{row.createdAt ? row.createdAt.toLocaleString() : "—"}</TableCell>
                      <TableCell className="text-muted-foreground">
      ...
      <AlertDialog open={confirmDeleteAllHistory} onOpenChange={setConfirmDeleteAllHistory}>
        <AlertDialogContent>
          <AlertDialogTitle>Delete all device history?</AlertDialogTitle>
          <AlertDialogDescription>
            This will permanently remove all entries from device history. This cannot be undone.
          </AlertDialogDescription>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction
