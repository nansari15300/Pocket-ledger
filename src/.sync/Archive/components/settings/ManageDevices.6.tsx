    ...
    <div className="space-y-8">
                ...
                {sortedDevices.map((device) => {
                  ...
                  <TableRow key={device.id}>
                    <TableCell className="text-sm" title={device.id}>
                      {device.deviceLabel || (device.deviceType === "mobile" ? "Mobile" : device.deviceType === "desktop" ? "Desktop" : "Device") + " ..." + device.id.slice(-8)}
                    </TableCell>
                    ...
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {isThisDevice ? (
                          ...
                          >
                            {removingThisDevice ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}
                            <span className="ml-1">Remove this device</span>
                          </Button>
                        ) : (
                          <Button
                            variant="ghost"
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Log of devices that were removed or kicked. Set how many entries to keep; oldest are removed when over limit.
          </CardDescription>
        </CardHeader>
                  ...
                  return (
                    <TableRow key={row.id}>
                      <TableCell className="text-sm" title={row.deviceId}>
                        {row.deviceLabel || (row.deviceType === "mobile" ? "Mobile" : row.deviceType === "desktop" ? "Desktop" : "Device") + " ..." + row.deviceId.slice(-8)}
                      </TableCell>
      ...
      <AlertDialog open={confirmDeleteAllHistory} onOpenChange={setConfirmDeleteAllHistory}>
        <AlertDialogContent>
          ...
          <AlertDialogDescription>
            This will remove all {deviceHistory.length} history entries. This cannot be undone.
          </AlertDialogDescription>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction
