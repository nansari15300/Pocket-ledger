...
import { getOrCreateDeviceId, removeThisDevice, trimDeviceHistoryToLimit, addDeviceHistoryEntryWhenRemoved, enforceSingleDevicePerUser, trimDevicesToPlanLimit } from "@/lib/deviceLimitClient";
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(devicesRef, (snap) => {
      ...
      if (rows.length > maxDevices) {
        trimDevicesToPlanLimit(companyId, maxDevices).then((removed) => {
          if (removed > 0) refreshDeviceCheck();
        });
      }
    });
    return () => unsub();
  }, [companyId, maxDevices, refreshDeviceCheck]);
