...
function EntriesChangesRows({ from, to, renderValue }: { from: any[]; to: any[]; renderValue: (v: any) => React.ReactNode }) {
  ...
  const pushSubFields = (rawOld: any, rawNew: any, prefix: string) => {
    ...
    for (const key of keys) {
      ...
      rows.push({ key: `${prefix}-${key}`, label: labels[key] ?? capitalizeFirst(key), oldVal, newVal });
    }
  };
  ...
  //    If paired, sub-fields are shown merged (old from removed, new from paired added) in one row each.
  //    If unpaired, sub-fields show old values with N/A new.
  for (const rawOld of removedArr) {
    ...
    // Merge sub-fields: old from this removed entry, new from paired added entry (if any)
    pushSubFields(rawOld, pairedNew, `${id}-paired`);
  }

  // 3. Added entries — if paired, ALL rows already shown above → skip entirely.
  //    If unpaired, show "Staff Member" row + sub-fields.
  for (const rawNew of addedArr) {
    const id = rawNew.accountId;
    if (pairedAddedIds.has(id)) continue; // already shown via paired removed entry above
    const { type } = resolveEntryInfo(id);
    const labels = ENTRY_FIELD_LABELS[type] ?? ENTRY_FIELD_LABELS.other;
    rows.push({
      key: `${id}-accountId`,
