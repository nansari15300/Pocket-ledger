...
function EntriesChangesRows({ from, to, renderValue }: { from: any[]; to: any[]; renderValue: (v: any) => React.ReactNode }) {
  const { processedAccounts, processedStaff, processedParties, expenseAccounts } = useVouchers();
  ...
  const resolveAccountName = (id: string) => {
    ...
    if (expense) return expense.name || expense.accountName;
    return id;
  };
  ...
  const prepareEntry = (entry: any) => {
    ...
    const prepared: any = { ...entry, accountId: resolveAccountName(entry.accountId) };
    if (typeof prepared.narration === 'string') prepared.narration = cleanNarration(prepared.narration);
    return prepared;
  };

  const changedEntries = allAccountIds.map((id: string) => {
    const oldEntry = (from || []).find((e: any) => e.accountId === id);
    const newEntry = (to || []).find((e: any) => e.accountId === id);
    if (JSON.stringify(oldEntry) === JSON.stringify(newEntry)) return null;
    return { id, oldEntry: prepareEntry(oldEntry), newEntry: prepareEntry(newEntry) };
  }).filter(Boolean);
  ...
  return (
    <>
      {changedEntries.map(({ id, oldEntry, newEntry }: any) => {
        ...
        const accountName = resolveAccountName(id);
        if (changedKeys.length === 0) return null;
        return (
          <React.Fragment key={id}>
            <TableRow className="bg-amber-50/50">
              <TableCell className="font-medium align-top min-w-[160px] w-1/3 text-left pr-2.5 whitespace-normal break-words">
                <span className="font-semibold">Entry: {accountName}</span>
              </TableCell>
              <TableCell className="bg-red-50/30 min-w-[140px] w-1/3 align-top text-left px-2.5" />
              <TableCell className="bg-green-50/30 min-w-[140px] w-1/3 align-top text-left pl-2.5" />
            </TableRow>
