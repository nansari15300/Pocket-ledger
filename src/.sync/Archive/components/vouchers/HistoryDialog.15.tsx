  ...
  return (
    <>
      {changedEntries.map(({ id, oldEntry, newEntry }: any) => {
        ...
        const { label, name } = resolveEntry(id);
        const accountLabel = label ? `${label}: ${name}` : name;
        if (changedKeys.length === 0) return null;
        return (
          <React.Fragment key={id}>
            {changedKeys.map((key) => (
              <TableRow key={key} className="bg-amber-50">
                <TableCell className="font-medium min-w-[160px] w-1/3 align-top text-left pr-2.5 whitespace-normal break-words">
                  <span className="font-semibold text-amber-800">{accountLabel}</span>
                  <span className="mx-1 text-muted-foreground">—</span>
                  <span>{capitalizeFirst(key)}</span>
                </TableCell>
...
export function HistoryDialog({ voucher, isOpen, onOpenChange, onHistoryReset, highlightTimestamp, highlightUid }: { voucher: any; isOpen: boolean; onOpenChange: (v: boolean) => void; onHistoryReset?: () => void; highlightTimestamp?: any; highlightUid?: string; }) {
   ...
   * Matches within 60 seconds of the notification timestamp.
   */
  const highlightIndex = useMemo(() => {
    if (!highlightTimestamp || groupedHistory.length === 0) return -1;
    const hlTime = highlightTimestamp?.toDate instanceof Function
      ? highlightTimestamp.toDate().getTime()
      : typeof highlightTimestamp === 'number' ? highlightTimestamp
      : new Date(highlightTimestamp).getTime();
    if (!hlTime || isNaN(hlTime)) return -1;
    ...
    groupedHistory.forEach((entry: any, idx: number) => {
      ...
      if (diff <= 60000 && uidMatch && diff < bestDiff) {
        bestDiff = diff;
        bestIdx = idx;
      }
    });
    // Fallback: if no uid match found, try closest time only
    if (bestIdx === -1) {
      groupedHistory.forEach((entry: any, idx: number) => {
        const entryTime = getHistoryChangedAt(entry.changedAt)?.getTime() ?? 0;
        const diff = Math.abs(entryTime - hlTime);
  ...
  // Scroll highlighted entry into center view when dialog opens
  useEffect(() => {
              ...
              groupedHistory.map((entry: any, i: number) => {
                ...
                <div
                  ...
                  className={cn(
                    ...
                    isHighlighted && "ring-2 ring-blue-500 shadow-lg shadow-blue-100",
                    isDimmed && "opacity-35"
                  )}
                >
