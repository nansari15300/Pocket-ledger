...
export async function registerDeviceAndCheckLimit(
  ...
  let totalCount = allDevicesSnap.size;
  let allDocs = allDevicesSnap.docs;
  if (totalCount > maxDevices) {
    await trimDevicesToPlanLimit(companyId, maxDevices);
    const refetch = await getDocs(collection(firestore, "companies", companyId, "devices"));
    allDocs = refetch.docs;
    totalCount = refetch.size;
  }
  const existing = allDocs.find((d) => d.id === deviceId);
  const myOtherDevices = allDocs.filter((d) => d.data()?.userId === userId && d.id !== deviceId);
...
/**
 * If device count exceeds plan limit, remove oldest devices (by lastActive) until count <= maxDevices.
 * Adds removed devices to device_history. Call whenever we need to enforce plan limit (e.g. on check or when owner views list).
 */
export async function trimDevicesToPlanLimit(companyId: string, maxDevices: number): Promise<number> {
  if (maxDevices < 1) return 0;
  const snap = await getDocs(collection(firestore, "companies", companyId, "devices"));
  if (snap.size <= maxDevices) return 0;
  const docs = snap.docs.map((d) => {
    ...
    const deviceType = data?.deviceType === "mobile" || data?.deviceType === "desktop" ? data.deviceType : undefined;
    const deviceLabel = typeof data?.deviceLabel === "string" ? data.deviceLabel : undefined;
    return { id: d.id, userId: data?.userId ?? "", lastActiveMs, deviceType, deviceLabel };
  });
  docs.sort((a, b) => a.lastActiveMs - b.lastActiveMs);
  const toRemove = docs.slice(0, docs.length - maxDevices);
  let removed = 0;
...
/**
 * When "User Can Use Multi Device" is turned off: for each user, keep only the device with the
 * latest lastActive and remove the rest. Adds removed devices to device_history.
 */
export async function enforceSingleDevicePerUser(companyId: string): Promise<number> {
  const snap = await getDocs(collection(firestore, "companies", companyId, "devices"));
  const byUser = new Map<string, { id: string; userId: string; lastActiveMs: number; deviceType?: "mobile" | "desktop"; deviceLabel?: string }[]>();
  for (const d of snap.docs) {
    ...
    byUser.get(userId)!.push({ id: d.id, userId, lastActiveMs, deviceType, deviceLabel });
  }
  let removed = 0;
  for (const [, devices] of byUser) {
    if (devices.length <= 1) continue;
    devices.sort((a, b) => b.lastActiveMs - a.lastActiveMs);
