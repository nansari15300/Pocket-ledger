...
export async function registerDeviceAndCheckLimit(
  ...
  const effectiveMax = maxDevices < 1 ? 1 : !hasMultiDeviceSync ? 1 : maxDevices;
  if (maxDevices < 1) {
  ...
  if (kickedSnap.exists()) {
    return { allowed: false, count: allDevicesSnap.size, limit: maxDevices, isNewDevice: false, wasKicked: true };
  }
  ...
  if (existing) {
    if (totalCount > maxDevices) {
      ...
      return { allowed: false, count: totalCount, limit: maxDevices, isNewDevice: false, replaceOffer: replaceOffer || undefined };
    }
