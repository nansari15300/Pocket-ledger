...
async function addDeviceHistoryEntry(companyId: string, deviceId: string, userId: string, deviceType: "mobile" | "desktop"): Promise<void> {
  try {
...
/**
 * Human-readable device label from User-Agent (e.g. "Chrome (Windows)", "Safari (iPhone)").
...
export function getDeviceLabel(): string {
  ...
  if (isMobile) {
    ...
    if (/Android/i.test(ua)) {
      ...
      return m ? `${browser} (${m[1].trim()})` : `${browser} (Android)`;
    }
...
export async function registerDeviceAndCheckLimit(
  ...
  if (existing) {
    ...
    addDeviceHistoryEntry(companyId, deviceId, userId, deviceType);
