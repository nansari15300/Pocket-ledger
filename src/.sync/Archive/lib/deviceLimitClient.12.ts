...
export async function registerDeviceAndCheckLimit(
  ...
  const isOwner = companyData?.ownerId === userId || (companyData?.ownerEmail != null && companyData.ownerEmail !== "" && options?.isOwner === true);
  const isOwnerOrShared = options?.isOwner === true || companyData?.ownerId === userId;
  ...
  if (!userCanUseMultiDevice && !isOwnerOrShared && !existing && myOtherDevices.length >= 1) {
  ...
  // Slot full: if User Can Use Multi Device = Off and this shared user has other devices, replace one and allow
  if (totalCount >= maxDevices && !userCanUseMultiDevice && !isOwnerOrShared && myOtherDevices.length >= 1) {
    try {
      await doReplaceAndRegister();
      return { allowed: true, count: totalCount - myOtherDevices.length + 1, limit: maxDevices, isNewDevice: true };
    } catch {
