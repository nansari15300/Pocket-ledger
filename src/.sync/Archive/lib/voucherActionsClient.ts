...
/** Normalise a fileUrls field value (string | string[] | null) to a flat string[]. */
...
/** Try to delete a Firebase Storage file by its download URL; swallows errors silently. */
async function tryDeleteStorageFile(url: string): Promise<void> {
  try {
    // refFromURL works for both https:// download URLs and gs:// paths
    const fileRef = storageRef(storage, url);
    await deleteObject(fileRef);
  } catch {
    // File might already be deleted or URL might be external — ignore
  }
}

/** Delete specific history entries by their changedAt millisecond timestamps.
 *  Also deletes from Firebase Storage any file URLs that were removed from the
 *  voucher as part of those history entries and are no longer referenced anywhere. */
export async function deleteHistoryEntries(
  ...
  // --- Find file URLs that were DELETED (in `from` but not in `to`) in the removed entries ---
  ...
  for (const entry of entriesToDelete) {
    for (const field of ['fileUrls', 'fileUrl', 'url']) {
      ...
      for (const url of fromUrls) {
        if (!toUrls.has(url)) candidateDeletedUrls.add(url);
      }
    }
  }

  if (candidateDeletedUrls.size > 0) {
    // Build set of all URLs still referenced (current voucher doc + remaining history entries)
    const stillReferenced = new Set<string>();
    for (const f of ['fileUrls', 'fileUrl', 'url']) {
      for (const u of toUrlArray(data[f])) stillReferenced.add(u);
    }
    ...
    // Delete from Storage only URLs no longer referenced anywhere
    await Promise.all(
      [...candidateDeletedUrls]
        .filter(url => !stillReferenced.has(url))
        .map(url => tryDeleteStorageFile(url))
    );
  }
