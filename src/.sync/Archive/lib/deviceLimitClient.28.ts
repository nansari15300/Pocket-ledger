...
/**
 * Trim device_history to at most `maxEntries`. Deletes oldest entries in batches until count <= maxEntries.
 */
export async function trimDeviceHistoryToLimit(companyId: string, maxEntries: number): Promise<void> {
  ...
  while (true) {
    ...
    const toDelete = snap.size <= maxEntries ? snap.docs.slice(0, snap.size - maxEntries) : snap.docs;
