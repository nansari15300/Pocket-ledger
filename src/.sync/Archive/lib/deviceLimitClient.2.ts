...
export async function registerDeviceAndCheckLimit(
  ...
  // History only on device change: this is a new device (first time using this company from this device).
...
export async function replaceMyOtherDevicesAndRegister(companyId: string, userId: string): Promise<void> {
  ...
  // History only on device change: user chose to replace other device(s) with this one.
  addDeviceHistoryEntry(companyId, deviceId, userId, deviceType, getDeviceLabel());
}
