...
export async function deleteHistoryEntries(
  ...
  const snap = await getDoc(voucherRef);
  if (!snap.exists()) throw new Error("Voucher not found");
  const existing: any[] = snap.data().history ?? [];
  const toDelete = new Set(changedAtMs);
  const filtered = existing.filter((h: any) => {
    const ms = h.changedAt?.toDate instanceof Function
      ? h.changedAt.toDate().getTime()
      : h.changedAt?._seconds != null ? h.changedAt._seconds * 1000
      : typeof h.changedAt === 'number' ? h.changedAt : null;
