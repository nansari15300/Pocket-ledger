...
const HISTORY_COOLDOWN_MS = 60 * 1000; // 1 minute per company (avoid duplicate from tabs or 45s re-check)
const HISTORY_COOLDOWN_KEY = "pl_device_history_last";

async function addDeviceHistoryEntry(companyId: string, deviceId: string, userId: string, deviceType: "mobile" | "desktop", deviceLabel?: string): Promise<void> {
  try {
    ...
    // non-blocking; device flow continues
  }
}
...
export async function registerDeviceAndCheckLimit(
  ...
  if (existing) {
    ...
    // Do not add history here — same device reopening company would create a new entry every time. History only on new device or replace.
  ...
  addDeviceHistoryEntry(companyId, deviceId, userId, deviceType, getDeviceLabel());
...
/**
 * Trim device_history to at most `maxEntries`. Call from Settings when admin saves limit or on load.
 */
export async function trimDeviceHistoryToLimit(companyId: string, maxEntries: number): Promise<void> {
  ...
  const q = query(col, orderBy("createdAt", "asc"), limit(maxEntries + 200));
  const snap = await getDocs(q);
  if (snap.size <= maxEntries) return;
  const toDelete = snap.docs.slice(0, snap.size - maxEntries);
