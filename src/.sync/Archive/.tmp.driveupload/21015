...
export type DeviceLimitResult = {
  ...
  /** True when admin kicked this device; do not auto re-register until user clicks "Use this device". */
  wasKicked?: boolean;
};
...
export async function registerDeviceAndCheckLimit(
  ...
  const limit = !hasMultiDeviceSync ? 1 : maxDevices;

  const deviceId = getOrCreateDeviceId();
  if (!deviceId) return { allowed: true, count: 1, limit, isNewDevice: false };
  ...
  const kickedRef = doc(firestore, "companies", companyId, "kicked_devices", deviceId);
  const [companySnap, allDevicesSnap, kickedSnap] = await Promise.all([
    getDoc(doc(firestore, "companies", companyId)),
    getDocs(collection(firestore, "companies", companyId, "devices")),
    getDoc(kickedRef),
  ]);
  if (kickedSnap.exists()) {
    return { allowed: false, count: allDevicesSnap.size, limit, isNewDevice: false, wasKicked: true };
  }
  ...
  if (!userCanUseMultiDevice && !isOwner && !existing && myOtherDevices.length >= 1) {
    return { allowed: false, count: totalCount, limit, isNewDevice: true, noPermissionNewDevice: true };
  }

  if (existing) {
    if (totalCount > limit) {
      const replaceOffer = userCanUseMultiDevice && !isOwner && myOtherDevices.length >= 1;
      return { allowed: false, count: totalCount, limit, isNewDevice: false, replaceOffer: replaceOffer || undefined };
    }
    ...
    if (Date.now() - lastMs < fiveMinMs) {
      return { allowed: true, count: totalCount, limit, isNewDevice: false };
    }
    await setDoc(doc(firestore, "companies", companyId, "devices", deviceId), {
      userId,
      lastActive: serverTimestamp(),
  ...
  if (totalCount >= limit) {
    ...
    const replaceOffer = userCanUseMultiDevice && !isOwner && myOtherDevices.length >= 1;
    return { allowed: false, count: totalCount, limit, isNewDevice: true, replaceOffer: replaceOffer || undefined };
  }
  ...
  return { allowed: true, count: totalCount + 1, limit, isNewDevice: true };
}
...
/** Mark device as kicked so it does not auto re-register on next 45s poll. Call when admin kicks a device. */
export async function markDeviceKicked(companyId: string, deviceId: string): Promise<void> {
  try {
    await setDoc(doc(firestore, "companies", companyId, "kicked_devices", deviceId), { kickedAt: serverTimestamp() });
  } catch {
    // non-blocking
  }
}

/** Clear kicked flag for current device so it can register again (e.g. when user clicks "Use this device"). */
export async function clearKickedFlag(companyId: string): Promise<void> {
  const deviceId = getOrCreateDeviceId();
  if (!companyId || !deviceId) return;
  try {
    await deleteDoc(doc(firestore, "companies", companyId, "kicked_devices", deviceId));
  } catch {
    // non-blocking
  }
}
