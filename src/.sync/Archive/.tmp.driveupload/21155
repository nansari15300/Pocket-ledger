...
export function DeviceLimitProvider({ children }: { children: ReactNode }) {
  const { deviceLimitReached, singleDeviceOnly, replaceOffer, deviceCount, maxDevices, refreshDeviceCheck, performReplaceAndRefresh } = useDeviceLimit();
  return (
    <DeviceLimitContext.Provider value={{ deviceLimitReached, singleDeviceOnly, replaceOffer, deviceCount, maxDevices, refreshDeviceCheck, performReplaceAndRefresh }}>
      {children}
