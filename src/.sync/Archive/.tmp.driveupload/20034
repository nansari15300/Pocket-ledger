...
const FIELD_ORDER: Record<string, number> = {
  ...
  lineItems: 5,
  entries: 5, Entries: 5,
  // Narration rows from entries — rendered AFTER total (synthetic slot)
...
function EntriesChangesRows({ from, to, renderValue, narrationOnly = false }: { from: any[]; to: any[]; renderValue: (v: any) => React.ReactNode; narrationOnly?: boolean }) {
  ...
  const entryTypeSortBase = (type: string, key: string): number => {
    if (key === 'accountId') {
      ...
      if (type === 'tax')     return 2;
      return 3;
    }
    if (key === 'narration') return 10;
    if (key === 'credit')    return 4;
    if (key === 'debit')     return 5;
    return 6;
  };
  ...
  const pushSubFields = (rawOld: any, rawNew: any, prefix: string) => {
    ...
    for (const key of keys) {
      ...
      // narrationOnly mode: skip all non-narration fields
      if (narrationOnly && key !== 'narration') continue;
      // normal mode: skip narration (it goes in the __entries_narration__ slot)
      if (!narrationOnly && key === 'narration') continue;
  ...
  // 2. Removed entries — ONE "Staff Member" row (old→paired-new or old→N/A)
  //    If paired, sub-fields show old→new together (single merged row per field).
  //    If not paired, sub-fields show old→N/A.
  for (const rawOld of removedArr) {
    ...
    // Merge sub-fields: if paired, show old→new in one row
    pushSubFields(rawOld, pairedNew, `${id}-pair`);
  }

  // 3. Added entries — if paired, SKIP entirely (already fully shown above).
  //    If not paired, show "Staff Member" row + sub-fields.
  for (const rawNew of addedArr) {
    const id = rawNew.accountId;
    if (pairedAddedIds.has(id)) continue; // fully merged into removed entry's rows
    const { type } = resolveEntryInfo(id);
    const labels = ENTRY_FIELD_LABELS[type] ?? ENTRY_FIELD_LABELS.other;
    rows.push({
      key: `${id}-accountId`,
                  ...
                  <Table className="w-full min-w-[480px]" scrollContainer={false}>
                    ...
                    <TableBody>
                      {(() => {
                        ...
                        // Inject synthetic __entries_narration__ slot so staff narration renders after Total
                        const entriesField = baseEntries.find(([f]) => f === 'entries' || f === 'Entries');
                        const withSlot: [string, any][] = entriesField
                          ? [...baseEntries, ['__entries_narration__', entriesField[1]]]
                          : baseEntries;
                        return withSlot
                        ...
                        // entries narration only (rendered after Total)
