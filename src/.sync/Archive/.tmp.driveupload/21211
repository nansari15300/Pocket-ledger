...
export type DeviceLimitResult = {
  ...
  /** True when this device was kicked and must not auto re-add; show "Use this device again?" until user confirms. */
...
export function getKickedCompanyId(): string | null {
  if (typeof window === "undefined") return null;
  try {
    for (let i = 0; i < window.localStorage.length; i++) {
      const key = window.localStorage.key(i);
      if (key?.startsWith(KICKED_STORAGE_PREFIX) && window.localStorage.getItem(key) === "1") return key.slice(KICKED_STORAGE_PREFIX.length);
    }
  } catch {}
  return null;
}
export function setKickedForCompany(companyId: string): void {
  try { window.localStorage.setItem(KICKED_STORAGE_PREFIX + companyId, "1"); } catch {}
}
export function clearKickedForCompany(companyId: string): void {
  try { window.localStorage.removeItem(KICKED_STORAGE_PREFIX + companyId); } catch {}
}
