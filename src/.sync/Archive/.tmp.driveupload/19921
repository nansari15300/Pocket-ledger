...
function EntriesChangesRows({ from, to, renderValue }: { from: any[]; to: any[]; renderValue: (v: any) => React.ReactNode }) {
  const { processedAccounts, processedStaff, processedParties } = useVouchers();
  ...
  const changedEntries = allAccountIds.map((id: string) => {
    ...
    const makeDisplay = (entry: any) => entry ? { ...entry, accountId: resolveAccountName(entry.accountId) } : null;
    return { id, oldEntry: makeDisplay(oldEntry), newEntry: makeDisplay(newEntry) };
  }).filter(Boolean);
