...
const FIELD_ORDER: Record<string, number> = {
  ...
  // 3. Line items / journal entries (salary: staff member, tax account, amounts, narration inside)
  lineItems: 5,
  entries: 5, Entries: 5,
  // 4. Amounts
  subTotal: 6,
...
const ENTRY_FIELD_LABELS: Record<string, Record<string, string>> = {
  ...
  expense: { accountId: 'Salary Account', credit: 'Credit',        debit: 'Expense Amount', narration: 'Narration' },
...
function EntriesChangesRows({ from, to, renderValue }: { from: any[]; to: any[]; renderValue: (v: any) => React.ReactNode }) {
  ...
  // Separate entries that exist in both (modified) vs only in old (removed) vs only in new (added)
  const fromArr = from || [];
  const toArr   = to   || [];
  ...
  // Pair removed ↔ added of the SAME type (staff↔staff, tax↔tax) by position so they show on one row
  const paired: { oldEntry: any; newEntry: any }[] = [];
  const usedAddedIdx = new Set<number>();

  for (const removed of removedArr) {
    const { type: rType } = resolveEntryInfo(removed.accountId);
    const matchIdx = addedArr.findIndex((a: any, i: number) =>
      !usedAddedIdx.has(i) && resolveEntryInfo(a.accountId).type === rType
    );
    if (matchIdx !== -1) {
      usedAddedIdx.add(matchIdx);
      paired.push({ oldEntry: removed, newEntry: addedArr[matchIdx] });
    } else {
      paired.push({ oldEntry: removed, newEntry: null });
    }
  }
  // Remaining unpaired additions
  addedArr.forEach((a: any, i: number) => {
    if (!usedAddedIdx.has(i)) paired.push({ oldEntry: null, newEntry: a });
  });

  // Build flat rows: { key, label, oldVal, newVal }
  const rows: { key: string; label: string; oldVal: any; newVal: any }[] = [];

  const buildRows = (rawOld: any, rawNew: any, pairKey: string) => {
    ...
    const entryAdded   = !rawOld && !!rawNew;
    const entryRemoved = !!rawOld && !rawNew;
    const entryPaired  = !!rawOld && !!rawNew && rawOld.accountId !== rawNew.accountId;
    const showAccountRow = entryAdded || entryRemoved || entryPaired;
    ...
    const allKeys = [
      ...
      ])].filter(k => !ENTRY_FIELD_ORDER.includes(k)),
    ];

    for (const key of allKeys) {
      if (key === 'accountId') {
        if (!showAccountRow) continue;
        rows.push({
          key: `${pairKey}-accountId`,
          label: labels.accountId ?? 'Account',
          oldVal: rawOld ? resolveEntryInfo(rawOld.accountId).name : null,
          newVal: rawNew ? resolveEntryInfo(rawNew.accountId).name : null,
        });
        continue;
      }
      ...
      rows.push({ key: `${pairKey}-${key}`, label: labels[key] ?? capitalizeFirst(key), oldVal, newVal });
    }
  };

  // Modified entries (same accountId in both)
  for (const id of bothIds) {
    const rawOld = fromArr.find((e: any) => e.accountId === id);
    const rawNew = toArr.find((e: any) => e.accountId === id);
    buildRows(rawOld, rawNew, id);
  }
  // Paired / unpaired removed/added
  paired.forEach(({ oldEntry, newEntry }, i) => {
    buildRows(oldEntry, newEntry, `pair-${i}`);
  });
              ...
              groupedHistory.map((entry: any, i: number) => {
                ...
                >
                  ...
                  <Table className="w-full min-w-[480px]" scrollContainer={false}>
                    ...
                    <TableBody>
                      {(Object.entries(entry.changes) as [string, any][])
