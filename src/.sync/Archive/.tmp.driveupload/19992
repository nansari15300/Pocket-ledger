...
function EntriesChangesRows({ from, to, renderValue }: { from: any[]; to: any[]; renderValue: (v: any) => React.ReactNode }) {
  ...
  for (const id of allAccountIds) {
    const rawOld = (from || []).find((e: any) => e.accountId === id) ?? null;
    const rawNew = (to || []).find((e: any) => e.accountId === id) ?? null;
    ...
    // Collect all field keys in display order
    const allKeys = [
      ...ENTRY_FIELD_ORDER,
      ...[...new Set([
      ...
      // accountId row: only when entry added/removed
      if (k === 'accountId') return entryAdded || entryRemoved;
      return rawOld ? Object.keys(rawOld).includes(k) : true ||
             rawNew ? Object.keys(rawNew).includes(k) : true;
    });

    for (const key of allKeys) {
      let oldVal = rawOld ? (key === 'accountId' ? resolveEntryInfo(rawOld.accountId).name : rawOld[key] ?? null) : null;
      let newVal = rawNew ? (key === 'accountId' ? resolveEntryInfo(rawNew.accountId).name : rawNew[key] ?? null) : null;
      ...
      // Skip unchanged (except accountId on add/remove which is always shown)
      if (key !== 'accountId' && JSON.stringify(oldVal) === JSON.stringify(newVal)) continue;

      const label = labels[key] ?? capitalizeFirst(key);
      rows.push({ key: `${id}-${key}`, label, oldVal, newVal });
    }
  }
