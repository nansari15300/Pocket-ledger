...
export default function MessagesPage() {
  ...
  // Backfill allAppUsers with conversation participants so Chat tab can resolve names (fixes "Unknown User" when initial fetch misses them).
  const missingParticipantIds = useMemo(() => {
    if (!relevantChatUserIds.length) return [];
    return relevantChatUserIds.filter(
      (pid) => !allAppUsers.some((u: any) => u.id === pid || u.uid === pid)
    );
  }, [relevantChatUserIds, allAppUsers]);
  useEffect(() => {
    if (!missingParticipantIds.length || !user) return;
    let cancelled = false;
    (async () => {
      try {
        const ids = missingParticipantIds.slice(0, 30);
        const fetched: any[] = [];
        const q = query(collection(firestore, "users"), where("uid", "in", ids));
        const snap = await getDocs(q);
        if (cancelled) return;
        snap.docs.forEach((d) => {
          fetched.push({ id: d.id, uid: d.data()?.uid ?? d.id, ...d.data() });
        });
        const stillMissing = ids.filter((id) => !fetched.some((u) => (u.uid || u.id) === id));
        for (const pid of stillMissing) {
          if (cancelled) return;
          const byId = await getDoc(doc(firestore, "users", pid));
          if (byId.exists() && byId.data()) {
            const data = byId.data()!;
            fetched.push({ id: byId.id, uid: data.uid ?? byId.id, ...data });
          }
        }
        if (fetched.length === 0) return;
        setAllAppUsers((prev) => {
          const existing = new Set(prev.map((u: any) => u.uid || u.id));
          const toAdd = fetched.filter((u: any) => !existing.has(u.uid || u.id));
          if (toAdd.length === 0) return prev;
          return [...prev, ...toAdd];
        });
      } catch (e) {
        console.error("Failed to backfill chat users:", e);
      }
    })();
    return () => { cancelled = true; };
  }, [missingParticipantIds, user]);
