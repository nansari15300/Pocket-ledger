...
export default function MessagesPage() {
  ...
  useEffect(() => {
    if (!relevantChatUserIds.length || !user) return;
    const missingIds = relevantChatUserIds.filter(
    ...
    (async () => {
      try {
        const ids = missingIds.slice(0, 30);
        const q = query(collection(firestore, "users"), where("uid", "in", ids));
        const snap = await getDocs(q);
        if (cancelled) return;
        const fetched = snap.docs.map((d) => ({ id: d.id, ...d.data() } as any));
        const existingKeys = new Set(allAppUsers.map((u: any) => u.id + "|" + (u.uid ?? "")));
        const toAdd = fetched.filter((u: any) => !existingKeys.has(u.id + "|" + (u.uid ?? "")));
        if (toAdd.length > 0) {
          setAllAppUsers((prev) => {
            const byKey = new Map(prev.map((u: any) => [u.uid || u.id, u]));
            toAdd.forEach((u: any) => {
              const key = u.uid || u.id;
              if (!byKey.has(key)) byKey.set(key, u);
            });
            return [...prev, ...toAdd.filter((u: any) => !prev.some((p: any) => (p.uid || p.id) === (u.uid || u.id)))];
          });
        }
      } catch (e) {
  ...
  }, [relevantChatUserIds, user, allAppUsers]);
