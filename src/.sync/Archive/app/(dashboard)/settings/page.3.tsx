...
const SETTINGS_SCROLL_KEY = "settingsPageScroll";
...
export default function SettingsPage() {
    ...
    const initialView = useMemo(() => {
        ...
        if (view && settingsNavItems.some((item) => item.id === view)) return view;
    ...
    useEffect(() => {
        const view = searchParams.get("view");
        if (view && settingsNavItems.some((item) => item.id === view) && view !== activeView) {
            setActiveView(view);
        }
    }, [searchParams]);
    ...
    // Pass view from URL so hook does not overwrite with localStorage on refresh
    usePageMemory(
    ...
    // Remember scroll position per tab (sessionStorage); restore on mount / tab change
    useEffect(() => {
        const el = mainScrollRef.current;
        if (!el) return;
        const key = `${SETTINGS_SCROLL_KEY}_${activeView}`;
