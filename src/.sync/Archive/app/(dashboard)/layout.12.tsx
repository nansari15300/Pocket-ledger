...
function DeviceLimitOverlay() {
  ...
  const [replaceTargetDevice, setReplaceTargetDevice] = useState<DeviceItem | null>(null);
  ...
  if (kickedAndBlocked && !isCompanyOwner) {
    ...
    return (
      <div className="absolute inset-0 z-50 flex flex-col items-center justify-center gap-4 bg-background/95 backdrop-blur-sm p-6 overflow-y-auto">
        ...
        <p className="text-center font-medium text-lg">You were removed from this company&apos;s devices.</p>
  ...
  return (
    <div className="absolute inset-0 z-50 flex flex-col items-center justify-center gap-4 bg-background/95 backdrop-blur-sm p-6 overflow-y-auto">
      ...
      <div className="w-full max-w-sm rounded-lg border bg-muted/50 px-3 py-2 text-left text-xs text-muted-foreground space-y-1">
        <p><span className="font-medium text-foreground">User name:</span> {user?.displayName?.trim() || "—"}</p>
        ...
        <p><span className="font-medium text-foreground">This device:</span> {typeof window !== "undefined" ? (getDeviceLabel() || "—") : "—"}</p>
      </div>
      ...
      {showReplaceOfferDialog && (
        <div className="w-full max-w-md space-y-4">
          {!showReplaceDeviceSelect ? (
            ...
            <div className="rounded-lg border bg-muted/30 px-3 py-2 text-left">
              <p className="text-xs font-semibold text-foreground mb-2">Select which device to remove. This device will be added in its place and you will be redirected to company.</p>
              <ul className="text-xs text-muted-foreground space-y-2 max-h-40 overflow-y-auto">
                {myDevices.map((d) => (
                  <li key={d.id} className="flex items-center justify-between gap-2 py-1 border-b border-border/50 last:border-0">
                    <span className="flex items-center gap-1.5 min-w-0">
                    ...
                    <Button
                      ...
                      onClick={() => setReplaceTargetDevice(d)}
                      disabled={!!kickingId}
                    >
          ...
          <AlertDialog open={!!replaceTargetDevice} onOpenChange={(open) => !open && setReplaceTargetDevice(null)}>
            <AlertDialogContent>
              <AlertDialogTitle>Remove that device and add this one?</AlertDialogTitle>
              <AlertDialogDescription>
                The selected device will be removed from the company. This device will be added in its place and you will be redirected to company.
              </AlertDialogDescription>
              <AlertDialogFooter>
                <AlertDialogCancel>Cancel</AlertDialogCancel>
                <AlertDialogAction
                  className="bg-destructive text-destructive-foreground hover:bg-destructive/90"
                  onClick={() => {
                    if (replaceTargetDevice) {
                      handleKickOut(replaceTargetDevice, () => {
                        refreshDeviceCheck();
                        router.push("/company");
                      });
      ...
      {!showReplaceOfferDialog && noPermissionNewDevice && !isCompanyOwner ? (
        <>
          <p className="text-center text-muted-foreground text-sm max-w-sm">
            No permission to use multi device. You can only use one device. Contact company admin or switch company.
          </p>
          <div className="flex flex-wrap items-center justify-center gap-3">
            <Link
              href="/company"
