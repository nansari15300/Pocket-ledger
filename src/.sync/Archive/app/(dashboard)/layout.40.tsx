...
import { getOrCreateDeviceId, getDeviceLabel, removeThisDevice, markDeviceKicked } from "@/lib/deviceLimitClient";
...
type DeviceItem = { id: string; userId: string; lastActive: string; ts: number; deviceType?: "mobile" | "desktop"; deviceLabel?: string };
...
function DeviceLimitOverlay() {
  ...
  const { deviceLimitReached, singleDeviceOnly, replaceOffer, noPermissionNewDevice, wasKicked, deviceCount, maxDevices, refreshDeviceCheck, clearKickedAndRefresh, performReplaceAndRefresh } = useDeviceLimitContext();
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(devicesRef, (snap) => {
      const list: DeviceItem[] = snap.docs.map((d) => {
        ...
        return { id: d.id, userId: data?.userId ?? "", lastActive, ts, deviceType, deviceLabel };
      });
      list.sort((a, b) => {
  ...
  const handleKickOut = async (device: DeviceItem, onSuccess?: () => void) => {
    ...
    try {
      await markDeviceKicked(companyId, device.id);
  ...
  const showReplaceOfferDialog = replaceOffer && !noPermissionNewDevice && !wasKicked && !isCompanyOwner;
  ...
  return (
    <div className="absolute inset-0 z-50 flex flex-col items-center justify-center gap-4 bg-background/95 backdrop-blur-sm p-6 overflow-y-auto">
      ...
      <p className="text-center font-medium text-lg">
        {wasKicked ? "You were removed from this company" : `Device limit reached (${deviceCount}/${maxDevices})`}
      </p>
      <div className="w-full max-w-sm rounded-lg border bg-muted/50 px-3 py-2 text-left text-xs text-muted-foreground space-y-1">
        ...
        <p><span className="font-medium text-foreground">This device:</span> {getDeviceLabel() || deviceIdShort || "—"}</p>
      </div>

      {wasKicked && !isCompanyOwner && (
        <>
          <p className="text-center text-muted-foreground text-sm max-w-sm">
            Admin removed this device. To use this company again from this device, click below (only if a slot is free).
          </p>
          <div className="flex flex-wrap items-center justify-center gap-3">
            <Button onClick={() => clearKickedAndRefresh()} className="rounded-md px-4 py-2 text-sm">
              Use this device
            </Button>
            <Link href="/company" className="inline-flex items-center justify-center rounded-md border border-input bg-background px-4 py-2 text-sm font-medium">
              Change company
            </Link>
      ...
      {!wasKicked && noPermissionNewDevice && !isCompanyOwner && (
        <>
          <p className="text-center text-muted-foreground text-sm max-w-sm font-medium text-foreground">
            No permission to change to new device by company admin.
          </p>
          <p className="text-center text-muted-foreground text-sm max-w-sm">
            Only the device(s) already registered can use this company. Change company or logout.
          </p>
          <div className="flex flex-wrap items-center justify-center gap-3">
            <Link
              href="/company"
      ...
      {!wasKicked && showReplaceOfferDialog && (
        <div className="w-full max-w-md rounded-lg border bg-muted/30 p-4 space-y-4">
          <p className="text-sm font-medium text-foreground text-center">
            Slot full. Your sessions on these devices — remove the one you want to stop using so you can use this device.
          </p>
          <ul className="text-xs text-muted-foreground space-y-2 max-h-48 overflow-y-auto">
            {myDevices.map((d) => (
              <li key={d.id} className="flex items-center justify-between gap-2 py-2 border-b border-border/50 last:border-0">
                <span className="flex items-center gap-1.5 min-w-0">
                  {d.deviceType === "mobile" ? <Smartphone className="h-3.5 w-3.5 shrink-0" /> : <Monitor className="h-3.5 w-3.5 shrink-0" />}
                  <span className="truncate">{d.deviceLabel || (d.deviceType === "mobile" ? "Mobile" : d.deviceType === "desktop" ? "Desktop" : `...${d.id.slice(-8)}`)}</span>
                  <span className="shrink-0 text-muted-foreground hidden sm:inline">{d.lastActive}</span>
                </span>
                <Button
                  size="sm"
                  variant="destructive"
          ...
          <p className="text-xs text-muted-foreground text-center">
            Or{" "}
            <button type="button" onClick={() => router.push("/company")} className="font-medium text-foreground underline hover:no-underline">
              switch company
            </button>
          </p>
        </div>
      ...
      {!wasKicked && !showReplaceOfferDialog && !noPermissionNewDevice && isCompanyOwner && activeDevices.length > 0 && (
            ...
            {activeDevices.map((d) => (
              <li key={d.id} className="flex items-center justify-between gap-2 py-1 border-b border-border/50 last:border-0">
                <span className="flex items-center gap-1.5 min-w-0">
                  ...
                  <span className="truncate text-muted-foreground">{d.deviceLabel || (d.deviceType === "mobile" ? "Mobile" : d.deviceType === "desktop" ? "Desktop" : `...${d.id.slice(-8)}`)}</span>
                </span>
      ...
      {!wasKicked && !showReplaceOfferDialog && !noPermissionNewDevice && !isCompanyOwner && myDevices.length > 0 && !singleDeviceOnly && (
        <div className="w-full max-w-md rounded-lg border bg-muted/30 px-3 py-2 text-left">
      ...
      {!wasKicked && !showReplaceOfferDialog && !noPermissionNewDevice && singleDeviceOnly && !isCompanyOwner && myDevices.length > 0 && (
        <div className="w-full max-w-md rounded-lg border bg-muted/30 px-3 py-2 text-left">
      ...
      {!wasKicked && !showReplaceOfferDialog && !noPermissionNewDevice && isCompanyOwner ? (
        <>
          <p className="text-center text-muted-foreground text-sm max-w-sm">
      ...
      ) : !wasKicked && !showReplaceOfferDialog && !noPermissionNewDevice && singleDeviceOnly ? (
        <p className="text-center text-muted-foreground text-sm max-w-sm">
          Log out from this device or use &quot;Replace with this device&quot; above to use this device instead.
        </p>
      ) : !wasKicked && !showReplaceOfferDialog && !noPermissionNewDevice && isNewUserNoSlot ? (
        <>
          <p className="text-center text-muted-foreground text-sm max-w-sm">
      ...
      ) : !wasKicked && !showReplaceOfferDialog && !noPermissionNewDevice ? (
        <p className="text-center text-muted-foreground text-sm max-w-sm">
