...
export function recurringAutoVoucherLabels(mode: CalendarDisplayMode) {
  if (mode === "AD") {
    return {
      settingsIntro:
        ...
        "Save saves the template; Generate now (when allowed) refills missing months or uses today as described below " +
        ...
        "Generate now fills the earliest missing BS month up to today (vouchers in the recycle bin count as missing).",
  ...
  if (mode === "Both") {
    return {
      settingsIntro:
        ...
        "when using yearly mode, then optional start date. Save saves the template; Generate now (when allowed) refills missing months or uses today as described below " +
        "(delete before the scheduled day → can run again on that day; delete on or after that day → no auto recreate for that BS month). " +
        "Journal: same debit/credit account lines share one template — Auto on/off on any month’s voucher applies to the whole series. " +
        "Generate now fills the earliest missing BS month up to today (vouchers in the recycle bin count as missing).",
      scheduleMonthDayLabel: "BS month day (generate on)",
  ...
  return {
    settingsIntro:
      ...
      "Use yearly mode to pick BS month and day, then optional start date on the calendar. Save saves the template; " +
      "Generate now (when allowed) refills missing months or uses today as described below (delete before the scheduled day → can run again on that day; delete on or after that day → no auto recreate for that BS month). " +
      "Journal: same debit/credit account lines share one template — Auto on/off on any month’s voucher applies to the whole series. " +
      "Generate now fills the earliest missing BS month up to today (vouchers in the recycle bin count as missing).",
    scheduleMonthDayLabel: "BS month day (generate on)",
    lastDayOfScheduledMonth: "Last day of BS month",
